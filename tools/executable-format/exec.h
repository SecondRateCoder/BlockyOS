...
#define MAGICDEF(name, nItems) typedef union name{char MAGIC[nItems];    size_t MAGICINT[nItems / sizeof(size_t)];}name;
MAGICDEF(SYMMAGIC, 16);
...
typedef struct exech{
	SYMMAGIC magic;        // exechmagic
	SYMMAGIC vermagic; // exechvermagic
	uint64_t attributes;
	uint64_t imageBase;
	uint16_t nSections;
