...
void *readblocks(rawenv re, LBA pos, size_t bytes){
    ...
    size_t blockBytes = re->RealBlock * re->CalcBlock;
    size_t nBlocks    = __safediv((bytes + blockBytes - 1), re->RealBlock);
    size_t allocSize  = nBlocks * re->RealBlock;
#ifdef __DEBUG__
...
void *writeblocks(rawenv re, void *data, LBA pos, size_t bytes){
	...
	size_t nBlocks = __safediv((bytes + (re->RealBlock * re->CalcBlock) - 1), re->RealBlock);
...
void writebytes(rawenv re, void *data, size_t bytepos, size_t nbytes){
	...
	size_t byteoffset = (bytepos * re->CalcBlock) % re->ConfBlock;
