		...
		for(size_t i = 0; i < largeroot->clusterBuffer.clusterSize; ++i){
			...
			if(f->fcode != 0){
				if(flagcheck(f->attr, __fsmetadatacluster) && f->fcode != 0){
					conf_fsblock *temp = readblock(re, largeroot->loc + 2 + largeroot->clusterBuffer.clusterSectors + (((size_t)f - (size_t)largeroot->clusterBuffer.fs) / sizeof(fsblock)), 1);
					if(memcmp(temp->fsig, FRATBLOCKSIG, 8)){
						printf("\nERROR!\nCorrupted FileSystem Root Block\nERASING ENTRY!!");
						memset(temp, 0, 512);
						f->fcode = 0;
						writeblock(re, temp, largeroot->loc + 2 + largeroot->clusterBuffer.clusterSectors + (((size_t)f - (size_t)largeroot->clusterBuffer.fs) / sizeof(fsblock)), 1);
					}
					free(temp);
				}
			}
...
void __fuloaddir(dirhandle *handle){
	...
	do{
		...
		if(f){writeblock(re, handle->dirarray + (getblocksize() * (entry - 1)), handle->root->loc + 2 + handle->root->clusterBuffer.clusterSectors + (((size_t)f - (size_t)handle->root->clusterBuffer.fs) / sizeof(fsblock)), 1);}
		entry++;
	}while(f);
	dispose(re);
