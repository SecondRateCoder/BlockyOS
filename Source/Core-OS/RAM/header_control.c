...
header_t header_decode(uint8_t *src, size_t Offset, pID_t ProgramID, ID_t ID){
    if(!adrress_validate(Offset)){return (header_t){.Data = NULL, .Context = NULL};}else{
...
uint8_t *header_encodeRAM(header_t H){memcpy(RAM, Pointer, header_encode(H), sizeof(H));}
...
bool adrress_validate(size_t Index){
    int cc =0;
    while(cc < NumberOfBlocks){
        if(decode_size_t(HeaderMetadata->Data, cc) == Index){return true;}
    }
    return false;
}
...
size_t *used_bounds(){
    size_t cc =0;
    size_t LowestAddress =0, HighestAddress =0;
    while(cc < NumberOfBlocks){
        if(HeaderMetadata->Data[cc] < LowestAddress){LowestAddress = HeaderMetadata->Data[cc];}
        if(HeaderMetadata->Data[cc] > HighestAddress){HighestAddress = HeaderMetadata->Data[cc];}
        ++cc;
    }
    return (size_t[2]){LowestAddress, HighestAddress};
}
size_t usedmemory(){
    size_t cc =0;
    size_t LowestAddress =0, HighestAddress =0;
...
bool used_at(size_t address){
    if(!adrress_validate(address)){return false;}
