...
#define bfd_arm_allocate_interworking_sections 	bfd_arm_wince_pe_allocate_interworking_sections
#define bfd_arm_get_bfd_for_interworking 	bfd_arm_wince_pe_get_bfd_for_interworking
#define bfd_arm_process_before_allocation 	bfd_arm_wince_pe_process_before_allocation
#include "coff-arm.h"
#endif
...
#define DEFAULT_DLL_CHARACTERISTICS	(0 ? 0 : 					   IMAGE_DLL_CHARACTERISTICS_DYNAMIC_BASE 					 | IMAGE_DLL_CHARACTERISTICS_NX_COMPAT)
...
is_underscoring (void)
{
  ...
  if (!bfd_get_target_info ("pei-arm-wince-little", NULL, NULL, &u, NULL))
    bfd_get_target_info ("pe-arm-wince-little", NULL, NULL, &u, NULL);
...
static bool
gldarm_wince_pe_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
static void
gldarm_wince_pe_before_plugin_all_symbols_read (void)
{
#ifdef DLL_SUPPORT
...
gldarm_wince_pe_get_script (int *isfile)
{
  *isfile = 1;
  ...
  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/arm_wince_pe.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/arm_wince_pe.xr";
  else if (!config.text_read_only)
    return "ldscripts/arm_wince_pe.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/arm_wince_pe.xn";
  else
...
struct ld_emulation_xfer_struct ld_arm_wince_pe_emulation =
{
  ...
  gldarm_wince_pe_before_plugin_all_symbols_read,
