...
/* 64 bit ELF emulation code for elf64kvx
...
static void gldelf64kvx_before_parse (void);
static void gldelf64kvx_before_plugin_all_symbols_read
  (void);
static void gldelf64kvx_after_open (void);
static void gldelf64kvx_before_allocation (void);
static void gldelf64kvx_after_allocation (void);
...
static void
gldelf64kvx_after_allocation (void)
{
  int ret;
...
static void
gldelf64kvx_before_plugin_all_symbols_read (void)
{
...
static void
gldelf64kvx_before_allocation (void)
{
...
static char *
gldelf64kvx_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/elf64kvx.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/elf64kvx.xr";
  else if (!config.text_read_only)
    return "ldscripts/elf64kvx.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/elf64kvx.xn";
  else if (bfd_link_dll (&link_info) && link_info.combreloc
	   && link_info.relro && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/elf64kvx.xswe";
      else
	return "ldscripts/elf64kvx.xsw";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/elf64kvx.xsce";
      else
	return "ldscripts/elf64kvx.xsc";
    }
  else if (bfd_link_dll (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/elf64kvx.xse";
      else
	return "ldscripts/elf64kvx.xs";
    }
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/elf64kvx.xwe";
      else
	return "ldscripts/elf64kvx.xw";
    }
  else if (link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/elf64kvx.xce";
      else
	return "ldscripts/elf64kvx.xc";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/elf64kvx.xe";
      else
	return "ldscripts/elf64kvx.x";
    }
}
...
static bool
gldelf64kvx_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
struct ld_emulation_xfer_struct ld_elf64kvx_emulation =
{
  gldelf64kvx_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldelf64kvx_before_plugin_all_symbols_read,
  gldelf64kvx_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldelf64kvx_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  elf64_kvx_before_allocation,
  gldelf64kvx_get_script,
  "elf64kvx",
  "elf64-kvx",
  gldelf64kvx_finish,
