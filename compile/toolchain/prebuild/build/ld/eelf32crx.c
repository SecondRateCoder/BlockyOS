...
/* 32 bit ELF emulation code for elf32crx
...
static void gldelf32crx_before_parse (void);
static void gldelf32crx_before_plugin_all_symbols_read
  (void);
static void gldelf32crx_after_open (void);
static void gldelf32crx_before_allocation (void);
static void gldelf32crx_after_allocation (void);
...
static void
gldelf32crx_before_parse (void)
{
  ldfile_set_output_arch ("crx", bfd_arch_crx);
...
static void
gldelf32crx_before_plugin_all_symbols_read (void)
{
...
static void
gldelf32crx_before_allocation (void)
{
...
static void
gldelf32crx_after_allocation (void)
{
...
static char *
gldelf32crx_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/elf32crx.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/elf32crx.xr";
  else if (!config.text_read_only)
    return "ldscripts/elf32crx.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/elf32crx.xn";
  else
    {
      if (link_info.separate_code)
	return "ldscripts/elf32crx.xe";
      else
	return "ldscripts/elf32crx.x";
    }
}
...
static bool
gldelf32crx_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
struct ld_emulation_xfer_struct ld_elf32crx_emulation =
{
  gldelf32crx_before_parse,
  syslib_default,
  hll_default,
  crxelf_after_parse,
  gldelf32crx_before_plugin_all_symbols_read,
  gldelf32crx_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldelf32crx_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  crxelf_before_allocation,
  gldelf32crx_get_script,
  "elf32crx",
  "elf32-crx",
  finish_default,
  NULL,
