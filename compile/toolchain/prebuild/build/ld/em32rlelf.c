...
/* 32 bit ELF emulation code for m32rlelf
...
static void gldm32rlelf_before_parse (void);
static void gldm32rlelf_before_plugin_all_symbols_read
  (void);
static void gldm32rlelf_after_open (void);
static void gldm32rlelf_before_allocation (void);
static void gldm32rlelf_after_allocation (void);


static void
gldm32rlelf_before_parse (void)
{
...
static void
gldm32rlelf_before_plugin_all_symbols_read (void)
{
...
static void
gldm32rlelf_before_allocation (void)
{
...
static void
gldm32rlelf_after_allocation (void)
{
...
static char *
gldm32rlelf_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/m32rlelf.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/m32rlelf.xr";
  else if (!config.text_read_only)
    return "ldscripts/m32rlelf.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/m32rlelf.xn";
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/m32rlelf.xwe";
      else
	return "ldscripts/m32rlelf.xw";
    }
  else if (link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/m32rlelf.xce";
      else
	return "ldscripts/m32rlelf.xc";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/m32rlelf.xe";
      else
	return "ldscripts/m32rlelf.x";
    }
}
...
static bool
gldm32rlelf_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
struct ld_emulation_xfer_struct ld_m32rlelf_emulation =
{
  gldm32rlelf_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldm32rlelf_before_plugin_all_symbols_read,
  gldm32rlelf_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldm32rlelf_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  gldm32rlelf_before_allocation,
  gldm32rlelf_get_script,
  "m32rlelf",
  "elf32-m32rle",
  finish_default,
  NULL,
