...
/* 32 bit ELF emulation code for elf64bpf
...
static void gldelf64bpf_before_parse (void);
static void gldelf64bpf_before_plugin_all_symbols_read
  (void);
static void gldelf64bpf_after_open (void);
static void gldelf64bpf_before_allocation (void);
static void gldelf64bpf_after_allocation (void);


static void
gldelf64bpf_before_parse (void)
{
  ldfile_set_output_arch ("bpf", bfd_arch_bpf);
...
static void
gldelf64bpf_before_plugin_all_symbols_read (void)
{
...
static void
gldelf64bpf_before_allocation (void)
{
...
static void
gldelf64bpf_after_allocation (void)
{
...
static char *
gldelf64bpf_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/elf64bpf.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/elf64bpf.xr";
  else if (!config.text_read_only)
    return "ldscripts/elf64bpf.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/elf64bpf.xn";
  else
    {
      if (link_info.separate_code)
	return "ldscripts/elf64bpf.xe";
      else
	return "ldscripts/elf64bpf.x";
    }
}
...
static bool
gldelf64bpf_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
struct ld_emulation_xfer_struct ld_elf64bpf_emulation =
{
  gldelf64bpf_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldelf64bpf_before_plugin_all_symbols_read,
  gldelf64bpf_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldelf64bpf_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  gldelf64bpf_before_allocation,
  gldelf64bpf_get_script,
  "elf64bpf",
  "elf64-bpfle",
  finish_default,
  NULL,
