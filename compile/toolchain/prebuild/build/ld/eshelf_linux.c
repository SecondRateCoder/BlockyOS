...
/* 32 bit ELF emulation code for shelf_linux
...
static void gldshelf_linux_before_parse (void);
static void gldshelf_linux_before_plugin_all_symbols_read
  (void);
static void gldshelf_linux_after_open (void);
static void gldshelf_linux_before_allocation (void);
static void gldshelf_linux_after_allocation (void);


static void
gldshelf_linux_before_parse (void)
{
...
static void
gldshelf_linux_before_plugin_all_symbols_read (void)
{
...
static void
gldshelf_linux_after_open (void)
{
...
static void
gldshelf_linux_before_allocation (void)
{
...
static void
gldshelf_linux_after_allocation (void)
{
...
static char *
gldshelf_linux_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/shelf_linux.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/shelf_linux.xr";
  else if (!config.text_read_only)
    return "ldscripts/shelf_linux.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/shelf_linux.xn";
	...
	return "ldscripts/shelf_linux.xdwe";
      else
	return "ldscripts/shelf_linux.xdw";
    }
  else if (bfd_link_pie (&link_info)
	   && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/shelf_linux.xdce";
      else
	return "ldscripts/shelf_linux.xdc";
    }
  else if (bfd_link_pie (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/shelf_linux.xde";
      else
	return "ldscripts/shelf_linux.xd";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc
	   && link_info.relro && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/shelf_linux.xswe";
      else
	return "ldscripts/shelf_linux.xsw";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/shelf_linux.xsce";
      else
	return "ldscripts/shelf_linux.xsc";
    }
  else if (bfd_link_dll (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/shelf_linux.xse";
      else
	return "ldscripts/shelf_linux.xs";
    }
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/shelf_linux.xwe";
      else
	return "ldscripts/shelf_linux.xw";
    }
  else if (link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/shelf_linux.xce";
      else
	return "ldscripts/shelf_linux.xc";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/shelf_linux.xe";
      else
	return "ldscripts/shelf_linux.x";
    }
}
...
static bool
gldshelf_linux_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
struct ld_emulation_xfer_struct ld_shelf_linux_emulation =
{
  gldshelf_linux_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldshelf_linux_before_plugin_all_symbols_read,
  gldshelf_linux_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldshelf_linux_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  gldshelf_linux_before_allocation,
  gldshelf_linux_get_script,
  "shelf_linux",
  "elf32-shbig-linux",
  finish_default,
  NULL,
