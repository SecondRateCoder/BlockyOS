...
/* 64 bit ELF emulation code for hppa64linux
...
static void gldhppa64linux_before_parse (void);
static void gldhppa64linux_before_plugin_all_symbols_read
  (void);
static void gldhppa64linux_after_open (void);
static void gldhppa64linux_before_allocation (void);
static void gldhppa64linux_after_allocation (void);


static void
gldhppa64linux_before_parse (void)
{
  ldfile_set_output_arch ("hppa:hppa2.0w", bfd_arch_hppa);
...
static void
gldhppa64linux_before_plugin_all_symbols_read (void)
{
...
static void
gldhppa64linux_after_open (void)
{
...
static void
gldhppa64linux_before_allocation (void)
{
...
static void
gldhppa64linux_after_allocation (void)
{
...
static char *
gldhppa64linux_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/hppa64linux.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/hppa64linux.xr";
  else if (!config.text_read_only)
    return "ldscripts/hppa64linux.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/hppa64linux.xn";
  else if (bfd_link_dll (&link_info) && link_info.combreloc
	   && link_info.relro && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/hppa64linux.xswe";
      else
	return "ldscripts/hppa64linux.xsw";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/hppa64linux.xsce";
      else
	return "ldscripts/hppa64linux.xsc";
    }
  else if (bfd_link_dll (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/hppa64linux.xse";
      else
	return "ldscripts/hppa64linux.xs";
    }
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/hppa64linux.xwe";
      else
	return "ldscripts/hppa64linux.xw";
    }
  else if (link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/hppa64linux.xce";
      else
	return "ldscripts/hppa64linux.xc";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/hppa64linux.xe";
      else
	return "ldscripts/hppa64linux.x";
    }
}
...
static bool
gldhppa64linux_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
struct ld_emulation_xfer_struct ld_hppa64linux_emulation =
{
  gldhppa64linux_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldhppa64linux_before_plugin_all_symbols_read,
  gldhppa64linux_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldhppa64linux_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  gldhppa64linux_before_allocation,
  gldhppa64linux_get_script,
  "hppa64linux",
  "elf64-hppa-linux",
  finish_default,
  NULL,
