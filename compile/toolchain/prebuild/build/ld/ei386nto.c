...
/* 32 bit ELF emulation code for i386nto
...
static void gldi386nto_before_parse (void);
static void gldi386nto_before_plugin_all_symbols_read
  (void);
static void gldi386nto_after_open (void);
static void gldi386nto_before_allocation (void);
static void gldi386nto_after_allocation (void);
...
static void
gldi386nto_before_plugin_all_symbols_read (void)
{
...
static void
gldi386nto_before_allocation (void)
{
...
static void
gldi386nto_after_allocation (void)
{
...
static char *
gldi386nto_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/i386nto.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/i386nto.xr";
  else if (!config.text_read_only)
    return "ldscripts/i386nto.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/i386nto.xn";
  else if (bfd_link_dll (&link_info) && link_info.combreloc
	   && link_info.relro && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/i386nto.xswe";
      else
	return "ldscripts/i386nto.xsw";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/i386nto.xsce";
      else
	return "ldscripts/i386nto.xsc";
    }
  else if (bfd_link_dll (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/i386nto.xse";
      else
	return "ldscripts/i386nto.xs";
    }
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/i386nto.xwe";
      else
	return "ldscripts/i386nto.xw";
    }
  else if (link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/i386nto.xce";
      else
	return "ldscripts/i386nto.xc";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/i386nto.xe";
      else
	return "ldscripts/i386nto.x";
    }
}
...
static bool
gldi386nto_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
struct ld_emulation_xfer_struct ld_i386nto_emulation =
{
  gldi386nto_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldi386nto_before_plugin_all_symbols_read,
  gldi386nto_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldi386nto_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  gldi386nto_before_allocation,
  gldi386nto_get_script,
  "i386nto",
  "elf32-i386",
  finish_default,
