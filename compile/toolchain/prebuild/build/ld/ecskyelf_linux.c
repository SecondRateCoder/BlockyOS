...
/* 32 bit ELF emulation code for cskyelf_linux
...
#define TARGET_IS_cskyelf_linux
...
static void gldcskyelf_linux_before_parse (void);
static void gldcskyelf_linux_before_plugin_all_symbols_read
  (void);
static void gldcskyelf_linux_after_open (void);
static void gldcskyelf_linux_before_allocation (void);
static void gldcskyelf_linux_after_allocation (void);
...
static void
gldcskyelf_linux_after_allocation (void)
{
...
static void
gldcskyelf_linux_before_parse (void)
{
  ldfile_set_output_arch ("csky", bfd_arch_csky);
...
static void
gldcskyelf_linux_before_plugin_all_symbols_read (void)
{
...
static void
gldcskyelf_linux_after_open (void)
{
...
static void
gldcskyelf_linux_before_allocation (void)
{
...
static char *
gldcskyelf_linux_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/cskyelf_linux.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/cskyelf_linux.xr";
  else if (!config.text_read_only)
    return "ldscripts/cskyelf_linux.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/cskyelf_linux.xn";
	...
	return "ldscripts/cskyelf_linux.xdwe";
      else
	return "ldscripts/cskyelf_linux.xdw";
    }
  else if (bfd_link_pie (&link_info)
	   && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/cskyelf_linux.xdce";
      else
	return "ldscripts/cskyelf_linux.xdc";
    }
  else if (bfd_link_pie (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/cskyelf_linux.xde";
      else
	return "ldscripts/cskyelf_linux.xd";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc
	   && link_info.relro && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/cskyelf_linux.xswe";
      else
	return "ldscripts/cskyelf_linux.xsw";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/cskyelf_linux.xsce";
      else
	return "ldscripts/cskyelf_linux.xsc";
    }
  else if (bfd_link_dll (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/cskyelf_linux.xse";
      else
	return "ldscripts/cskyelf_linux.xs";
    }
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/cskyelf_linux.xwe";
      else
	return "ldscripts/cskyelf_linux.xw";
    }
  else if (link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/cskyelf_linux.xce";
      else
	return "ldscripts/cskyelf_linux.xc";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/cskyelf_linux.xe";
      else
	return "ldscripts/cskyelf_linux.x";
    }
}
...
static bool
gldcskyelf_linux_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
static void
gldcskyelf_linux_list_options (FILE * file)
{
...
struct ld_emulation_xfer_struct ld_cskyelf_linux_emulation =
{
  gldcskyelf_linux_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldcskyelf_linux_before_plugin_all_symbols_read,
  gldcskyelf_linux_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldcskyelf_linux_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  gldcskyelf_linux_before_allocation,
  gldcskyelf_linux_get_script,
  "cskyelf_linux",
  "elf32-csky-little",
  gldcskyelf_linux_finish,
  csky_elf_create_output_section_statements,
  ldelf_open_dynamic_archive,
  ldelf_place_orphan,
  NULL,
  NULL,
  gldcskyelf_linux_add_options,
  gldcskyelf_linux_handle_option,
  NULL,
  gldcskyelf_linux_list_options,
  ldelf_load_symbols,
  NULL,
  NULL,
  NULL,
