...
/* 32 bit ELF emulation code for crislinux
...
static void gldcrislinux_before_parse (void);
static void gldcrislinux_before_plugin_all_symbols_read
  (void);
static void gldcrislinux_after_open (void);
static void gldcrislinux_before_allocation (void);
static void gldcrislinux_after_allocation (void);


static void
gldcrislinux_before_parse (void)
{
  ldfile_set_output_arch ("cris", bfd_arch_cris);
...
static void
gldcrislinux_before_plugin_all_symbols_read (void)
{
...
static void
gldcrislinux_before_allocation (void)
{
...
static void
gldcrislinux_after_allocation (void)
{
...
static char *
gldcrislinux_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/crislinux.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/crislinux.xr";
  else if (!config.text_read_only)
    return "ldscripts/crislinux.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/crislinux.xn";
  else if (bfd_link_dll (&link_info) && link_info.combreloc
	   && link_info.relro && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/crislinux.xswe";
      else
	return "ldscripts/crislinux.xsw";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/crislinux.xsce";
      else
	return "ldscripts/crislinux.xsc";
    }
  else if (bfd_link_dll (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/crislinux.xse";
      else
	return "ldscripts/crislinux.xs";
    }
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/crislinux.xwe";
      else
	return "ldscripts/crislinux.xw";
    }
  else if (link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/crislinux.xce";
      else
	return "ldscripts/crislinux.xc";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/crislinux.xe";
      else
	return "ldscripts/crislinux.x";
    }
}
...
static bool
gldcrislinux_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
struct ld_emulation_xfer_struct ld_crislinux_emulation =
{
  gldcrislinux_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldcrislinux_before_plugin_all_symbols_read,
  gldcrislinux_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldcrislinux_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  gldcrislinux_before_allocation,
  gldcrislinux_get_script,
  "crislinux",
  "elf32-cris",
  finish_default,
  NULL,
