...
/* 32 bit ELF emulation code for shelf_nbsd
...
static void gldshelf_nbsd_before_parse (void);
static void gldshelf_nbsd_before_plugin_all_symbols_read
  (void);
static void gldshelf_nbsd_after_open (void);
static void gldshelf_nbsd_before_allocation (void);
static void gldshelf_nbsd_after_allocation (void);


static void
gldshelf_nbsd_before_parse (void)
{
...
static void
gldshelf_nbsd_before_plugin_all_symbols_read (void)
{
...
static void
gldshelf_nbsd_before_allocation (void)
{
...
static void
gldshelf_nbsd_after_allocation (void)
{
...
static char *
gldshelf_nbsd_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/shelf_nbsd.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/shelf_nbsd.xr";
  else if (!config.text_read_only)
    return "ldscripts/shelf_nbsd.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/shelf_nbsd.xn";
  else if (bfd_link_dll (&link_info) && link_info.combreloc
	   && link_info.relro && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/shelf_nbsd.xswe";
      else
	return "ldscripts/shelf_nbsd.xsw";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/shelf_nbsd.xsce";
      else
	return "ldscripts/shelf_nbsd.xsc";
    }
  else if (bfd_link_dll (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/shelf_nbsd.xse";
      else
	return "ldscripts/shelf_nbsd.xs";
    }
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/shelf_nbsd.xwe";
      else
	return "ldscripts/shelf_nbsd.xw";
    }
  else if (link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/shelf_nbsd.xce";
      else
	return "ldscripts/shelf_nbsd.xc";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/shelf_nbsd.xe";
      else
	return "ldscripts/shelf_nbsd.x";
    }
}
...
static bool
gldshelf_nbsd_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
struct ld_emulation_xfer_struct ld_shelf_nbsd_emulation =
{
  gldshelf_nbsd_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldshelf_nbsd_before_plugin_all_symbols_read,
  gldshelf_nbsd_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldshelf_nbsd_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  gldshelf_nbsd_before_allocation,
  gldshelf_nbsd_get_script,
  "shelf_nbsd",
  "elf32-sh-nbsd",
  finish_default,
  NULL,
