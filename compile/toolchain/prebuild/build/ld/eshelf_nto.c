...
/* 32 bit ELF emulation code for shelf_nto
...
static void gldshelf_nto_before_parse (void);
static void gldshelf_nto_before_plugin_all_symbols_read
  (void);
static void gldshelf_nto_after_open (void);
static void gldshelf_nto_before_allocation (void);
static void gldshelf_nto_after_allocation (void);


static void
gldshelf_nto_before_parse (void)
{
...
static void
gldshelf_nto_before_plugin_all_symbols_read (void)
{
...
static void
gldshelf_nto_before_allocation (void)
{
...
static void
gldshelf_nto_after_allocation (void)
{
...
static char *
gldshelf_nto_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/shelf_nto.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/shelf_nto.xr";
  else if (!config.text_read_only)
    return "ldscripts/shelf_nto.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/shelf_nto.xn";
  else if (bfd_link_dll (&link_info) && link_info.combreloc
	   && link_info.relro && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/shelf_nto.xswe";
      else
	return "ldscripts/shelf_nto.xsw";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/shelf_nto.xsce";
      else
	return "ldscripts/shelf_nto.xsc";
    }
  else if (bfd_link_dll (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/shelf_nto.xse";
      else
	return "ldscripts/shelf_nto.xs";
    }
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/shelf_nto.xwe";
      else
	return "ldscripts/shelf_nto.xw";
    }
  else if (link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/shelf_nto.xce";
      else
	return "ldscripts/shelf_nto.xc";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/shelf_nto.xe";
      else
	return "ldscripts/shelf_nto.x";
    }
}
...
static bool
gldshelf_nto_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
struct ld_emulation_xfer_struct ld_shelf_nto_emulation =
{
  gldshelf_nto_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldshelf_nto_before_plugin_all_symbols_read,
  gldshelf_nto_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldshelf_nto_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  gldshelf_nto_before_allocation,
  gldshelf_nto_get_script,
  "shelf_nto",
  "elf32-sh",
  finish_default,
  NULL,
