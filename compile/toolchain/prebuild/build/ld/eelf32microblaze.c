...
/* 32 bit ELF emulation code for elf32microblaze
...
#define TARGET_IS_elf32microblaze
...
static void gldelf32microblaze_before_parse (void);
static void gldelf32microblaze_before_plugin_all_symbols_read
  (void);
static void gldelf32microblaze_after_open (void);
static void gldelf32microblaze_before_allocation (void);
static void gldelf32microblaze_after_allocation (void);


static void
gldelf32microblaze_before_parse (void)
{
...
static void
gldelf32microblaze_before_plugin_all_symbols_read (void)
{
...
static void
gldelf32microblaze_after_open (void)
{
...
static void
gldelf32microblaze_before_allocation (void)
{
...
static void
gldelf32microblaze_after_allocation (void)
{
...
static char *
gldelf32microblaze_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/elf32microblaze.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/elf32microblaze.xr";
  else if (!config.text_read_only)
    return "ldscripts/elf32microblaze.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/elf32microblaze.xn";
  else
    {
      if (link_info.separate_code)
	return "ldscripts/elf32microblaze.xe";
      else
	return "ldscripts/elf32microblaze.x";
    }
}
...
static void
gldelf32microblaze_add_options
...
static bool
gldelf32microblaze_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
struct ld_emulation_xfer_struct ld_elf32microblaze_emulation =
{
  gldelf32microblaze_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldelf32microblaze_before_plugin_all_symbols_read,
  gldelf32microblaze_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldelf32microblaze_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  gldelf32microblaze_before_allocation,
  gldelf32microblaze_get_script,
  "elf32microblaze",
  "elf32-microblaze",
  finish_default,
  NULL,
  ...
  NULL,
  gldelf32microblaze_add_options,
  gldelf32microblaze_handle_option,
  NULL,
  NULL,
  ldelf_load_symbols,
  NULL,
