...
static void gldaixppc_read_file (const char *, bool);
static void gldaixppc_free (void *);
static void gldaixppc_find_relocs (lang_statement_union_type *);
static void gldaixppc_find_exp_assignment (etree_type *);
...
static bool
gldaixppc_parse_args (int argc, char **argv)
{
  int indx;
...
static bool
gldaixppc_handle_option (int optc)
{
  bfd_signed_vma val;
  const char *end;
  ...
  switch (optc)
	...
	*t = 0;
    ...
    case OPTION_EXPORT:
      gldaixppc_read_file (optarg, false);
      break;
...
static bool
gldaixppc_unrecognized_file (lang_input_statement_type *entry)
{
  FILE *e;
  bool ret;
...
static void
gldaixppc_before_allocation (void)
{
  struct filelist *fl;
  ...
  for (fl = import_files; fl != NULL; fl = fl->next)
    gldaixppc_read_file (fl->name, true);
  ...
  lang_for_each_statement (gldaixppc_find_relocs);
...
static void
gldaixppc_after_allocation (void)
{
...
static char *
gldaixppc_choose_target (int argc, char **argv)
{
  int i, j, jmax;
...
static void
gldaixppc_read_file (const char *filename, bool import)
{
  struct obstack *o;
  FILE *f;
  int lineno;
  ...
  obstack_specify_allocation (o, 0, 0, xmalloc, gldaixppc_free);
...
static void
gldaixppc_find_relocs (lang_statement_union_type *s)
{
  ...
  if (s->header.type == lang_assignment_statement_enum)
    gldaixppc_find_exp_assignment (s->assignment_statement.exp);
}

/* Look through an expression for an assignment statement.  */

static void
gldaixppc_find_exp_assignment (etree_type *exp)
{
  struct bfd_link_hash_entry *h;
  ...
  switch (exp->type.node_class)
      ...
      if (strcmp (exp->assign.dst, ".") != 0)
	{
      ...
      gldaixppc_find_exp_assignment (exp->assign.src);
      break;

    case etree_binary:
      gldaixppc_find_exp_assignment (exp->binary.lhs);
      gldaixppc_find_exp_assignment (exp->binary.rhs);
      break;

    case etree_trinary:
      gldaixppc_find_exp_assignment (exp->trinary.cond);
      gldaixppc_find_exp_assignment (exp->trinary.lhs);
      gldaixppc_find_exp_assignment (exp->trinary.rhs);
      break;

    case etree_unary:
      gldaixppc_find_exp_assignment (exp->unary.child);
      break;
...
static char *
gldaixppc_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/aixppc.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/aixppc.xr";
  else if (!config.text_read_only)
    return "ldscripts/aixppc.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/aixppc.xn";
  else
    return "ldscripts/aixppc.x";
}

static void
gldaixppc_create_output_section_statements (void)
{
...
static void
gldaixppc_set_output_arch (void)
{
...
static bool
gldaixppc_open_dynamic_archive (const char *arch,
					   search_dirs_type *search,
...
static bool
gldaixppc_print_symbol (struct bfd_link_hash_entry *hash_entry,
				   void *ptr)
{
...
struct ld_emulation_xfer_struct ld_aixppc_emulation =
{
  ...
  after_parse_default,
  NULL,
  gldaixppc_after_open,
  after_check_relocs_default,
  before_place_orphans_default,
  gldaixppc_after_allocation,
  gldaixppc_set_output_arch,
  gldaixppc_choose_target,
  gldaixppc_before_allocation,
  gldaixppc_get_script,
  "aixppc",
  "aixcoff-rs6000",
  finish_default,
