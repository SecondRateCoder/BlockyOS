...
static void
gldarm64pe_list_options (FILE *file)
{
...
static bool
gldarm64pe_handle_option (int optc)
{
  is_underscoring ();
  switch (optc)
    {
...
static void
gldarm64pe_before_plugin_all_symbols_read (void)
{
#ifdef DLL_SUPPORT
...

static void
gldarm64pe_before_allocation (void)
{
  is_underscoring ();
...
static bool
gldarm64pe_unrecognized_file (lang_input_statement_type *entry ATTRIBUTE_UNUSED)
{
#ifdef DLL_SUPPORT
...
static bool
gldarm64pe_recognized_file (lang_input_statement_type *entry ATTRIBUTE_UNUSED)
{
  is_underscoring ();
#ifdef DLL_SUPPORT
#ifdef TARGET_IS_i386pep
...
gldarm64pe_place_orphan (asection *s,
				    const char *secname,
				    int constraint)
{
...
static bool
gldarm64pe_open_dynamic_archive
...
static int
gldarm64pe_find_potential_libraries
...

static char *
gldarm64pe_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/arm64pe.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/arm64pe.xr";
  else if (!config.text_read_only)
    return "ldscripts/arm64pe.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/arm64pe.xn";
  else if (link_info.pei386_auto_import == 1
	   && link_info.pei386_runtime_pseudo_reloc != 2)
    return "ldscripts/arm64pe.xa";
  else
    return "ldscripts/arm64pe.x";
}

struct ld_emulation_xfer_struct ld_arm64pe_emulation =
{
  gldarm64pe_before_parse,
  syslib_default,
  hll_default,
  gldarm64pe_after_parse,
  gldarm64pe_before_plugin_all_symbols_read,
  gldarm64pe_after_open,
  after_check_relocs_default,
  before_place_orphans_default,
  after_allocation_default,
  set_output_arch_default,
  ldemul_default_target,
  gldarm64pe_before_allocation,
  gldarm64pe_get_script,
  "arm64pe",
  "pei-aarch64-little",
  gldarm64pe_finish,
  NULL,
  gldarm64pe_open_dynamic_archive,
  gldarm64pe_place_orphan,
  gldarm64pe_set_symbols,
  NULL,
  gldarm64pe_add_options,
  gldarm64pe_handle_option,
  gldarm64pe_unrecognized_file,
  gldarm64pe_list_options,
  gldarm64pe_recognized_file,
  gldarm64pe_find_potential_libraries,
  NULL,
  NULL,
  NULL,
