...
/* 32 bit ELF emulation code for avr1
...
static void gldavr1_before_parse (void);
static void gldavr1_before_plugin_all_symbols_read
  (void);
static void gldavr1_after_open (void);
static void gldavr1_before_allocation (void);
static void gldavr1_after_allocation (void);
...
static void
avr_elf_avr1_before_allocation (void)
{
  int ret;

  gldavr1_before_allocation ();
  ...
  if (strcmp ("avr1", "avr6") != 0
      && strcmp ("avr1", "avrxmega6") != 0
      && strcmp ("avr1", "avrxmega7") != 0)
    avr_no_stubs = true;
...
static void
gldavr1_before_parse (void)
{
  ldfile_set_output_arch ("avr:1", bfd_arch_avr);
...
static void
gldavr1_before_plugin_all_symbols_read (void)
{
...
static void
gldavr1_before_allocation (void)
{
...
static char *
gldavr1_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/avr1.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/avr1.xr";
  else if (!config.text_read_only)
    return "ldscripts/avr1.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/avr1.xn";
  else
    {
      if (link_info.separate_code)
	return "ldscripts/avr1.xe";
      else
	return "ldscripts/avr1.x";
    }
}
...
static bool
gldavr1_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
static void
gldavr1_list_options (FILE * file)
{
...
struct ld_emulation_xfer_struct ld_avr1_emulation =
{
  avr_elf_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldavr1_before_plugin_all_symbols_read,
  gldavr1_after_open,
  ...
  avr_elf_avr1_before_allocation,
  gldavr1_get_script,
  "avr1",
  "elf32-avr",
  avr_finish,
  ...
  NULL,
  gldavr1_add_options,
  gldavr1_handle_option,
  NULL,
  gldavr1_list_options,
  ldelf_load_symbols,
  NULL,
  NULL,
  NULL,
