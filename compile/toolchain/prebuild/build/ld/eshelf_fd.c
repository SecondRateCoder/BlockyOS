...
/* 32 bit ELF emulation code for shelf_fd
...
static void gldshelf_fd_before_parse (void);
static void gldshelf_fd_before_plugin_all_symbols_read
  (void);
static void gldshelf_fd_after_open (void);
static void gldshelf_fd_before_allocation (void);
static void gldshelf_fd_after_allocation (void);


static void
gldshelf_fd_before_parse (void)
{
...
static void
gldshelf_fd_before_plugin_all_symbols_read (void)
{
...
static void
gldshelf_fd_before_allocation (void)
{
...
static void
gldshelf_fd_after_allocation (void)
{
...
static char *
gldshelf_fd_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/shelf_fd.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/shelf_fd.xr";
  else if (!config.text_read_only)
    return "ldscripts/shelf_fd.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/shelf_fd.xn";
	...
	return "ldscripts/shelf_fd.xdwe";
      else
	return "ldscripts/shelf_fd.xdw";
    }
  else if (bfd_link_pie (&link_info)
	   && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/shelf_fd.xdce";
      else
	return "ldscripts/shelf_fd.xdc";
    }
  else if (bfd_link_pie (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/shelf_fd.xde";
      else
	return "ldscripts/shelf_fd.xd";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc
	   && link_info.relro && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/shelf_fd.xswe";
      else
	return "ldscripts/shelf_fd.xsw";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/shelf_fd.xsce";
      else
	return "ldscripts/shelf_fd.xsc";
    }
  else if (bfd_link_dll (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/shelf_fd.xse";
      else
	return "ldscripts/shelf_fd.xs";
    }
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/shelf_fd.xwe";
      else
	return "ldscripts/shelf_fd.xw";
    }
  else if (link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/shelf_fd.xce";
      else
	return "ldscripts/shelf_fd.xc";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/shelf_fd.xe";
      else
	return "ldscripts/shelf_fd.x";
    }
}
...
static bool
gldshelf_fd_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
struct ld_emulation_xfer_struct ld_shelf_fd_emulation =
{
  gldshelf_fd_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldshelf_fd_before_plugin_all_symbols_read,
  gldshelf_fd_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldshelf_fd_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  gldshelf_fd_before_allocation,
  gldshelf_fd_get_script,
  "shelf_fd",
  "elf32-shbig-fdpic",
  finish_default,
  NULL,
