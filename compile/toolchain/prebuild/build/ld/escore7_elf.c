...
/* 32 bit ELF emulation code for score7_elf
...
static void gldscore7_elf_before_parse (void);
static void gldscore7_elf_before_plugin_all_symbols_read
  (void);
static void gldscore7_elf_after_open (void);
static void gldscore7_elf_before_allocation (void);
static void gldscore7_elf_after_allocation (void);


#include "elf32-score.h"

static void
gldscore7_elf_before_parse (void)
{
#ifndef TARGET_			/* I.e., if not generic.  */
  ldfile_set_output_arch ("score7", bfd_arch_unknown);
#endif /* not TARGET_ */
...
static void
gldscore7_elf_before_plugin_all_symbols_read (void)
{
...
static void
gldscore7_elf_before_allocation (void)
{
...
static void
gldscore7_elf_after_allocation (void)
{
...
static char *
gldscore7_elf_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/score7_elf.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/score7_elf.xr";
  else if (!config.text_read_only)
    return "ldscripts/score7_elf.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/score7_elf.xn";
  else if (bfd_link_dll (&link_info) && link_info.combreloc
	   && link_info.relro && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/score7_elf.xswe";
      else
	return "ldscripts/score7_elf.xsw";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/score7_elf.xsce";
      else
	return "ldscripts/score7_elf.xsc";
    }
  else if (bfd_link_dll (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/score7_elf.xse";
      else
	return "ldscripts/score7_elf.xs";
    }
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/score7_elf.xwe";
      else
	return "ldscripts/score7_elf.xw";
    }
  else if (link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/score7_elf.xce";
      else
	return "ldscripts/score7_elf.xc";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/score7_elf.xe";
      else
	return "ldscripts/score7_elf.x";
    }
}
...
static bool
gldscore7_elf_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
struct ld_emulation_xfer_struct ld_score7_elf_emulation =
{
  gldscore7_elf_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldscore7_elf_before_plugin_all_symbols_read,
  score_elf_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldscore7_elf_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  gldscore7_elf_before_allocation,
  gldscore7_elf_get_script,
  "score7_elf",
  "elf32-bigscore",
  finish_default,
  NULL,
