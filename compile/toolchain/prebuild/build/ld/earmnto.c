...
/* 32 bit ELF emulation code for armnto
...
static void gldarmnto_before_parse (void);
static void gldarmnto_before_plugin_all_symbols_read
  (void);
static void gldarmnto_after_open (void);
static void gldarmnto_before_allocation (void);
static void gldarmnto_after_allocation (void);
...
static void
gldarmnto_after_allocation (void)
{
  int ret;
...
static void
gldarmnto_before_plugin_all_symbols_read (void)
{
...
static void
gldarmnto_before_allocation (void)
{
...
static char *
gldarmnto_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/armnto.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/armnto.xr";
  else if (!config.text_read_only)
    return "ldscripts/armnto.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/armnto.xn";
  else if (bfd_link_dll (&link_info) && link_info.combreloc
	   && link_info.relro && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/armnto.xswe";
      else
	return "ldscripts/armnto.xsw";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/armnto.xsce";
      else
	return "ldscripts/armnto.xsc";
    }
  else if (bfd_link_dll (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/armnto.xse";
      else
	return "ldscripts/armnto.xs";
    }
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/armnto.xwe";
      else
	return "ldscripts/armnto.xw";
    }
  else if (link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/armnto.xce";
      else
	return "ldscripts/armnto.xc";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/armnto.xe";
      else
	return "ldscripts/armnto.x";
    }
}
...
static bool
gldarmnto_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
static void
gldarmnto_list_options (FILE * file)
{
...
struct ld_emulation_xfer_struct ld_armnto_emulation =
{
  gldarmnto_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldarmnto_before_plugin_all_symbols_read,
  gldarmnto_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldarmnto_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  arm_elf_before_allocation,
  gldarmnto_get_script,
  "armnto",
  "elf32-littlearm",
  gldarmnto_finish,
  ...
  ldelf_place_orphan,
  gldarmnto_set_symbols,
  NULL,
  gldarmnto_add_options,
  gldarmnto_handle_option,
  NULL,
  gldarmnto_list_options,
  ldelf_load_symbols,
  NULL,
  NULL,
  NULL,
