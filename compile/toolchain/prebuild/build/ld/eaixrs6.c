...
static void gldaixrs6_read_file (const char *, bool);
static void gldaixrs6_free (void *);
static void gldaixrs6_find_relocs (lang_statement_union_type *);
static void gldaixrs6_find_exp_assignment (etree_type *);
...
static bool
gldaixrs6_parse_args (int argc, char **argv)
{
  int indx;
...
static bool
gldaixrs6_handle_option (int optc)
{
  bfd_signed_vma val;
  const char *end;
  ...
  switch (optc)
	...
	*t = 0;
    ...
    case OPTION_EXPORT:
      gldaixrs6_read_file (optarg, false);
      break;
...
static bool
gldaixrs6_unrecognized_file (lang_input_statement_type *entry)
{
  FILE *e;
  bool ret;
...
static void
gldaixrs6_before_allocation (void)
{
  struct filelist *fl;
  ...
  for (fl = import_files; fl != NULL; fl = fl->next)
    gldaixrs6_read_file (fl->name, true);
  ...
  lang_for_each_statement (gldaixrs6_find_relocs);
...
static void
gldaixrs6_after_allocation (void)
{
...
static char *
gldaixrs6_choose_target (int argc, char **argv)
{
  int i, j, jmax;
...
static void
gldaixrs6_read_file (const char *filename, bool import)
{
  struct obstack *o;
  FILE *f;
  int lineno;
  ...
  obstack_specify_allocation (o, 0, 0, xmalloc, gldaixrs6_free);
...
static void
gldaixrs6_find_relocs (lang_statement_union_type *s)
{
  ...
  if (s->header.type == lang_assignment_statement_enum)
    gldaixrs6_find_exp_assignment (s->assignment_statement.exp);
}

/* Look through an expression for an assignment statement.  */

static void
gldaixrs6_find_exp_assignment (etree_type *exp)
{
  struct bfd_link_hash_entry *h;
  ...
  switch (exp->type.node_class)
      ...
      if (strcmp (exp->assign.dst, ".") != 0)
	{
      ...
      gldaixrs6_find_exp_assignment (exp->assign.src);
      break;

    case etree_binary:
      gldaixrs6_find_exp_assignment (exp->binary.lhs);
      gldaixrs6_find_exp_assignment (exp->binary.rhs);
      break;

    case etree_trinary:
      gldaixrs6_find_exp_assignment (exp->trinary.cond);
      gldaixrs6_find_exp_assignment (exp->trinary.lhs);
      gldaixrs6_find_exp_assignment (exp->trinary.rhs);
      break;

    case etree_unary:
      gldaixrs6_find_exp_assignment (exp->unary.child);
      break;
...
static char *
gldaixrs6_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/aixrs6.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/aixrs6.xr";
  else if (!config.text_read_only)
    return "ldscripts/aixrs6.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/aixrs6.xn";
  else
    return "ldscripts/aixrs6.x";
}

static void
gldaixrs6_create_output_section_statements (void)
{
...
static void
gldaixrs6_set_output_arch (void)
{
...
static bool
gldaixrs6_open_dynamic_archive (const char *arch,
					   search_dirs_type *search,
...
static bool
gldaixrs6_print_symbol (struct bfd_link_hash_entry *hash_entry,
				   void *ptr)
{
...
struct ld_emulation_xfer_struct ld_aixrs6_emulation =
{
  ...
  after_parse_default,
  NULL,
  gldaixrs6_after_open,
  after_check_relocs_default,
  before_place_orphans_default,
  gldaixrs6_after_allocation,
  gldaixrs6_set_output_arch,
  gldaixrs6_choose_target,
  gldaixrs6_before_allocation,
  gldaixrs6_get_script,
  "aixrs6",
  "aixcoff-rs6000",
  finish_default,
