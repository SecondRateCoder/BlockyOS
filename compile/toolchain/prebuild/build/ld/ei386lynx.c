...
/* 32 bit ELF emulation code for i386lynx
...
static void gldi386lynx_before_parse (void);
static void gldi386lynx_before_plugin_all_symbols_read
  (void);
static void gldi386lynx_after_open (void);
static void gldi386lynx_before_allocation (void);
static void gldi386lynx_after_allocation (void);
...
static void
gldi386lynx_before_plugin_all_symbols_read (void)
{
...
static void
gldi386lynx_before_allocation (void)
{
...
static void
gldi386lynx_after_allocation (void)
{
...
static char *
gldi386lynx_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/i386lynx.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/i386lynx.xr";
  else if (!config.text_read_only)
    return "ldscripts/i386lynx.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/i386lynx.xn";
  else if (bfd_link_dll (&link_info) && link_info.combreloc
	   && link_info.relro && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/i386lynx.xswe";
      else
	return "ldscripts/i386lynx.xsw";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/i386lynx.xsce";
      else
	return "ldscripts/i386lynx.xsc";
    }
  else if (bfd_link_dll (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/i386lynx.xse";
      else
	return "ldscripts/i386lynx.xs";
    }
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/i386lynx.xwe";
      else
	return "ldscripts/i386lynx.xw";
    }
  else if (link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/i386lynx.xce";
      else
	return "ldscripts/i386lynx.xc";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/i386lynx.xe";
      else
	return "ldscripts/i386lynx.x";
    }
}
...
static bool
gldi386lynx_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
struct ld_emulation_xfer_struct ld_i386lynx_emulation =
{
  gldi386lynx_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldi386lynx_before_plugin_all_symbols_read,
  gldi386lynx_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldi386lynx_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  gldi386lynx_before_allocation,
  gldi386lynx_get_script,
  "i386lynx",
  "elf32-i386",
  finish_default,
