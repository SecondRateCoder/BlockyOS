...
/* 64 bit ELF emulation code for elf64kvx_linux
...
#define TARGET_IS_elf64kvx_linux
...
static void gldelf64kvx_linux_before_parse (void);
static void gldelf64kvx_linux_before_plugin_all_symbols_read
  (void);
static void gldelf64kvx_linux_after_open (void);
static void gldelf64kvx_linux_before_allocation (void);
static void gldelf64kvx_linux_after_allocation (void);
...
static void
elf64_kvx_before_allocation (void)
{

  /* Call the standard elf routine.  */
  gldelf64kvx_linux_before_allocation ();
}
...
/* Call-back for elf64_kvx_size_stubs.  */

/* Create a new stub section, and arrange for it to be linked
   immediately after INPUT_SECTION.  */

static asection *
elf64_kvx_add_stub_section (const char *stub_sec_name,
                                    asection *input_section)
{
  asection *stub_sec;
  flagword flags;
...
/* Another call-back for elf64_kvx_size_stubs.  */
...
build_section_lists (lang_statement_union_type *statement)
{
  if (statement->header.type == lang_input_section_enum)
	...
	elf64_kvx_next_input_section (& link_info, i);
    }
}

static void
gldelf64kvx_linux_after_allocation (void)
{
  int ret;
  ...
  if (stub_file != NULL && !bfd_link_relocatable (&link_info))
    {
      ret = elf64_kvx_setup_section_lists (link_info.output_bfd,
						       &link_info);
      if (ret != 0)
	{
	  if (ret < 0)
	  ...
	  if (! elf64_kvx_size_stubs (link_info.output_bfd,
					  stub_file->the_bfd,
					  & link_info,
					  group_size,
					  & elf64_kvx_add_stub_section,
...
gldelf64kvx_linux_finish (void)
{
  if (!bfd_link_relocatable (&link_info))
	...
	{
	  if (! elf64_kvx_build_stubs (& link_info))
...
kvx_elf_create_output_section_statements (void)
{
  ...
  if (!kvx_elf64_init_stub_bfd (&link_info, stub_file->the_bfd))
...
static void
gldelf64kvx_linux_before_parse (void)
{
  ldfile_set_output_arch ("kvx", bfd_arch_kvx);
...
static void
gldelf64kvx_linux_before_plugin_all_symbols_read (void)
{
...
static void
gldelf64kvx_linux_after_open (void)
{
...
static void
gldelf64kvx_linux_before_allocation (void)
{
...
static char *
gldelf64kvx_linux_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/elf64kvx_linux.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/elf64kvx_linux.xr";
  else if (!config.text_read_only)
    return "ldscripts/elf64kvx_linux.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/elf64kvx_linux.xn";
	...
	return "ldscripts/elf64kvx_linux.xdwe";
      else
	return "ldscripts/elf64kvx_linux.xdw";
    }
  else if (bfd_link_pie (&link_info)
	   && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/elf64kvx_linux.xdce";
      else
	return "ldscripts/elf64kvx_linux.xdc";
    }
  else if (bfd_link_pie (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/elf64kvx_linux.xde";
      else
	return "ldscripts/elf64kvx_linux.xd";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc
	   && link_info.relro && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/elf64kvx_linux.xswe";
      else
	return "ldscripts/elf64kvx_linux.xsw";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/elf64kvx_linux.xsce";
      else
	return "ldscripts/elf64kvx_linux.xsc";
    }
  else if (bfd_link_dll (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/elf64kvx_linux.xse";
      else
	return "ldscripts/elf64kvx_linux.xs";
    }
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/elf64kvx_linux.xwe";
      else
	return "ldscripts/elf64kvx_linux.xw";
    }
  else if (link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/elf64kvx_linux.xce";
      else
	return "ldscripts/elf64kvx_linux.xc";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/elf64kvx_linux.xe";
      else
	return "ldscripts/elf64kvx_linux.x";
    }
}
...
static bool
gldelf64kvx_linux_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
struct ld_emulation_xfer_struct ld_elf64kvx_linux_emulation =
{
  gldelf64kvx_linux_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldelf64kvx_linux_before_plugin_all_symbols_read,
  gldelf64kvx_linux_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldelf64kvx_linux_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  elf64_kvx_before_allocation,
  gldelf64kvx_linux_get_script,
  "elf64kvx_linux",
  "elf64-kvx",
  gldelf64kvx_linux_finish,
  kvx_elf_create_output_section_statements,
  ldelf_open_dynamic_archive,
  ldelf_place_orphan,
  NULL,
  NULL,
  gldelf64kvx_linux_add_options,
  gldelf64kvx_linux_handle_option,
  NULL,
  NULL,
  ldelf_load_symbols,
  NULL,
