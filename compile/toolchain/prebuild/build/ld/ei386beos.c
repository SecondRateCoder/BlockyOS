...
static bool
gldi386beos_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
sort_by_file_name (const void *a, const void *b)
{
  ...
  /* the tail idata4/5 are the only ones without relocs to an
     idata section unless we are importing by ordinal,
  ...
  if (sa->reloc_count + sb->reloc_count != 0)
    ...
    {
      /* don't sort .idata or .idata FIXME dlltool eliminate .idata */
      if ((strcmp (sa->name, ".idata") == 0))
	return 0;
...
gldi386beos_place_orphan (asection *s,
				   const char *secname,
				   int constraint)
{
...
gldi386beos_get_script (int *isfile)
{
  *isfile = 1;
  ...
  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/i386beos.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/i386beos.xr";
  else if (!config.text_read_only)
    return "ldscripts/i386beos.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/i386beos.xn";
  else
