...
/* 32 bit ELF emulation code for m32rlelf_linux
...
#define TARGET_IS_m32rlelf_linux
...
static void gldm32rlelf_linux_before_parse (void);
static void gldm32rlelf_linux_before_plugin_all_symbols_read
  (void);
static void gldm32rlelf_linux_after_open (void);
static void gldm32rlelf_linux_before_allocation (void);
static void gldm32rlelf_linux_after_allocation (void);


static void
gldm32rlelf_linux_before_parse (void)
{
...
static void
gldm32rlelf_linux_before_plugin_all_symbols_read (void)
{
...
static void
gldm32rlelf_linux_after_open (void)
{
...
static void
gldm32rlelf_linux_before_allocation (void)
{
...
static void
gldm32rlelf_linux_after_allocation (void)
{
...
static char *
gldm32rlelf_linux_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/m32rlelf_linux.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/m32rlelf_linux.xr";
  else if (!config.text_read_only)
    return "ldscripts/m32rlelf_linux.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/m32rlelf_linux.xn";
	...
	return "ldscripts/m32rlelf_linux.xdwe";
      else
	return "ldscripts/m32rlelf_linux.xdw";
    }
  else if (bfd_link_pie (&link_info)
	   && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/m32rlelf_linux.xdce";
      else
	return "ldscripts/m32rlelf_linux.xdc";
    }
  else if (bfd_link_pie (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/m32rlelf_linux.xde";
      else
	return "ldscripts/m32rlelf_linux.xd";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc
	   && link_info.relro && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/m32rlelf_linux.xswe";
      else
	return "ldscripts/m32rlelf_linux.xsw";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/m32rlelf_linux.xsce";
      else
	return "ldscripts/m32rlelf_linux.xsc";
    }
  else if (bfd_link_dll (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/m32rlelf_linux.xse";
      else
	return "ldscripts/m32rlelf_linux.xs";
    }
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/m32rlelf_linux.xwe";
      else
	return "ldscripts/m32rlelf_linux.xw";
    }
  else if (link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/m32rlelf_linux.xce";
      else
	return "ldscripts/m32rlelf_linux.xc";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/m32rlelf_linux.xe";
      else
	return "ldscripts/m32rlelf_linux.x";
    }
}
...
static bool
gldm32rlelf_linux_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
struct ld_emulation_xfer_struct ld_m32rlelf_linux_emulation =
{
  gldm32rlelf_linux_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldm32rlelf_linux_before_plugin_all_symbols_read,
  gldm32rlelf_linux_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldm32rlelf_linux_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  gldm32rlelf_linux_before_allocation,
  gldm32rlelf_linux_get_script,
  "m32rlelf_linux",
  "elf32-m32rle-linux",
  finish_default,
  NULL,
  ...
  NULL,
  gldm32rlelf_linux_add_options,
  gldm32rlelf_linux_handle_option,
  NULL,
  NULL,
  ldelf_load_symbols,
  NULL,
