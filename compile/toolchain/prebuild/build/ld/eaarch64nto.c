...
/* 64 bit ELF emulation code for aarch64nto
...
static void gldaarch64nto_before_parse (void);
static void gldaarch64nto_before_plugin_all_symbols_read
  (void);
static void gldaarch64nto_after_open (void);
static void gldaarch64nto_before_allocation (void);
static void gldaarch64nto_after_allocation (void);
...
static void
gldaarch64nto_before_parse (void)
{
...
aarch64_elf_before_allocation (void)
{
  ...
  gldaarch64nto_before_allocation ();
}
...
static void
gldaarch64nto_after_allocation (void)
{
  int ret;
...
nto_create_QNX_note_section(int type)
{
  ...
  /* As aarch64elf.em is imported and aarch64_elf_create_output_section_statements
...
static void
gldaarch64nto_before_plugin_all_symbols_read (void)
{
...
static void
gldaarch64nto_before_allocation (void)
{
  ldelf_before_allocation (audit, depaudit, "/usr/lib/ldqnx-64.so.2");
}


static char *
gldaarch64nto_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/aarch64nto.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/aarch64nto.xr";
  else if (!config.text_read_only)
    return "ldscripts/aarch64nto.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/aarch64nto.xn";
	...
	return "ldscripts/aarch64nto.xdwe";
      else
	return "ldscripts/aarch64nto.xdw";
    }
  else if (bfd_link_pie (&link_info)
	   && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/aarch64nto.xdce";
      else
	return "ldscripts/aarch64nto.xdc";
    }
  else if (bfd_link_pie (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/aarch64nto.xde";
      else
	return "ldscripts/aarch64nto.xd";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc
	   && link_info.relro && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/aarch64nto.xswe";
      else
	return "ldscripts/aarch64nto.xsw";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/aarch64nto.xsce";
      else
	return "ldscripts/aarch64nto.xsc";
    }
  else if (bfd_link_dll (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/aarch64nto.xse";
      else
	return "ldscripts/aarch64nto.xs";
    }
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/aarch64nto.xwe";
      else
	return "ldscripts/aarch64nto.xw";
    }
  else if (link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/aarch64nto.xce";
      else
	return "ldscripts/aarch64nto.xc";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/aarch64nto.xe";
      else
	return "ldscripts/aarch64nto.x";
    }
}
...
static bool
gldaarch64nto_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
static void
gldaarch64nto_list_options (FILE * file)
{
...
struct ld_emulation_xfer_struct ld_aarch64nto_emulation =
{
  gldaarch64nto_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldaarch64nto_before_plugin_all_symbols_read,
  nto_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldaarch64nto_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  aarch64_elf_before_allocation,
  gldaarch64nto_get_script,
  "aarch64nto",
  "elf64-littleaarch64",
  gldaarch64nto_finish,
  ...
  NULL,
  gldaarch64nto_add_options,
  gldaarch64nto_handle_option,
  NULL,
  gldaarch64nto_list_options,
  ldelf_load_symbols,
  NULL,
  NULL,
  NULL,
