...
/* 32 bit ELF emulation code for shlelf_fd
...
static void gldshlelf_fd_before_parse (void);
static void gldshlelf_fd_before_plugin_all_symbols_read
  (void);
static void gldshlelf_fd_after_open (void);
static void gldshlelf_fd_before_allocation (void);
static void gldshlelf_fd_after_allocation (void);


static void
gldshlelf_fd_before_parse (void)
{
...
static void
gldshlelf_fd_before_plugin_all_symbols_read (void)
{
...
static void
gldshlelf_fd_before_allocation (void)
{
...
static void
gldshlelf_fd_after_allocation (void)
{
...
static char *
gldshlelf_fd_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/shlelf_fd.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/shlelf_fd.xr";
  else if (!config.text_read_only)
    return "ldscripts/shlelf_fd.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/shlelf_fd.xn";
	...
	return "ldscripts/shlelf_fd.xdwe";
      else
	return "ldscripts/shlelf_fd.xdw";
    }
  else if (bfd_link_pie (&link_info)
	   && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/shlelf_fd.xdce";
      else
	return "ldscripts/shlelf_fd.xdc";
    }
  else if (bfd_link_pie (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/shlelf_fd.xde";
      else
	return "ldscripts/shlelf_fd.xd";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc
	   && link_info.relro && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/shlelf_fd.xswe";
      else
	return "ldscripts/shlelf_fd.xsw";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/shlelf_fd.xsce";
      else
	return "ldscripts/shlelf_fd.xsc";
    }
  else if (bfd_link_dll (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/shlelf_fd.xse";
      else
	return "ldscripts/shlelf_fd.xs";
    }
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/shlelf_fd.xwe";
      else
	return "ldscripts/shlelf_fd.xw";
    }
  else if (link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/shlelf_fd.xce";
      else
	return "ldscripts/shlelf_fd.xc";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/shlelf_fd.xe";
      else
	return "ldscripts/shlelf_fd.x";
    }
}
...
static bool
gldshlelf_fd_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
struct ld_emulation_xfer_struct ld_shlelf_fd_emulation =
{
  gldshlelf_fd_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldshlelf_fd_before_plugin_all_symbols_read,
  gldshlelf_fd_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldshlelf_fd_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  gldshlelf_fd_before_allocation,
  gldshlelf_fd_get_script,
  "shlelf_fd",
  "elf32-sh-fdpic",
  finish_default,
  NULL,
