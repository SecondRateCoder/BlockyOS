...
#define bfd_arm_allocate_interworking_sections 	bfd_armpe_allocate_interworking_sections
#define bfd_arm_get_bfd_for_interworking 	bfd_armpe_get_bfd_for_interworking
#define bfd_arm_process_before_allocation 	bfd_armpe_process_before_allocation
#include "coff-arm.h"
#endif
...
is_underscoring (void)
{
  ...
  if (!bfd_get_target_info ("pei-arm-little", NULL, NULL, &u, NULL))
    bfd_get_target_info ("pe-arm-little", NULL, NULL, &u, NULL);
...
static bool
gldarmpe_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
static void
gldarmpe_before_plugin_all_symbols_read (void)
{
#ifdef DLL_SUPPORT
...
gldarmpe_get_script (int *isfile)
{
  *isfile = 1;
  ...
  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/armpe.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/armpe.xr";
  else if (!config.text_read_only)
    return "ldscripts/armpe.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/armpe.xn";
  else
...
struct ld_emulation_xfer_struct ld_armpe_emulation =
{
  ...
  gldarmpe_before_plugin_all_symbols_read,
  gldarmpe_after_open,
