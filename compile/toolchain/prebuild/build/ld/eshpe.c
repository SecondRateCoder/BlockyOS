...
#define bfd_arm_allocate_interworking_sections 	bfd_shpe_allocate_interworking_sections
#define bfd_arm_get_bfd_for_interworking 	bfd_shpe_get_bfd_for_interworking
#define bfd_arm_process_before_allocation 	bfd_shpe_process_before_allocation
#include "coff-arm.h"
#endif
...
is_underscoring (void)
{
  ...
  if (!bfd_get_target_info ("pei-shl", NULL, NULL, &u, NULL))
...
static bool
gldshpe_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
static void
gldshpe_before_plugin_all_symbols_read (void)
{
#ifdef DLL_SUPPORT
...
gldshpe_get_script (int *isfile)
{
  *isfile = 1;
  ...
  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/shpe.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/shpe.xr";
  else if (!config.text_read_only)
    return "ldscripts/shpe.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/shpe.xn";
  else
...
struct ld_emulation_xfer_struct ld_shpe_emulation =
{
  ...
  hll_default,
  gldshpe_after_parse,
  gldshpe_before_plugin_all_symbols_read,
  gldshpe_after_open,
