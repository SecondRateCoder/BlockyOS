...
#define bfd_arm_allocate_interworking_sections 	bfd_mcorepe_allocate_interworking_sections
#define bfd_arm_get_bfd_for_interworking 	bfd_mcorepe_get_bfd_for_interworking
#define bfd_arm_process_before_allocation 	bfd_mcorepe_process_before_allocation
#include "coff-arm.h"
#endif
...
is_underscoring (void)
{
  ...
  if (!bfd_get_target_info ("pei-mcore-little", NULL, NULL, &u, NULL))
    bfd_get_target_info ("", NULL, NULL, &u, NULL);
...
static bool
gldmcorepe_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
static void
gldmcorepe_before_plugin_all_symbols_read (void)
{
#ifdef DLL_SUPPORT
...
gldmcorepe_get_script (int *isfile)
{
  *isfile = 1;
  ...
  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/mcorepe.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/mcorepe.xr";
  else if (!config.text_read_only)
    return "ldscripts/mcorepe.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/mcorepe.xn";
  else
...
struct ld_emulation_xfer_struct ld_mcorepe_emulation =
{
  ...
  gldmcorepe_before_plugin_all_symbols_read,
