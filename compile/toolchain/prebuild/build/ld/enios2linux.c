...
/* 32 bit ELF emulation code for nios2linux
...
static void gldnios2linux_before_parse (void);
static void gldnios2linux_before_plugin_all_symbols_read
  (void);
static void gldnios2linux_after_open (void);
static void gldnios2linux_before_allocation (void);
static void gldnios2linux_after_allocation (void);
...
static void
gldnios2linux_after_allocation (void)
{
  int ret;
...
static void
gldnios2linux_before_parse (void)
{
  ldfile_set_output_arch ("nios2", bfd_arch_nios2);
...
static void
gldnios2linux_before_plugin_all_symbols_read (void)
{
...
static void
gldnios2linux_before_allocation (void)
{
...
static char *
gldnios2linux_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/nios2linux.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/nios2linux.xr";
  else if (!config.text_read_only)
    return "ldscripts/nios2linux.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/nios2linux.xn";
	...
	return "ldscripts/nios2linux.xdwe";
      else
	return "ldscripts/nios2linux.xdw";
    }
  else if (bfd_link_pie (&link_info)
	   && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/nios2linux.xdce";
      else
	return "ldscripts/nios2linux.xdc";
    }
  else if (bfd_link_pie (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/nios2linux.xde";
      else
	return "ldscripts/nios2linux.xd";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc
	   && link_info.relro && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/nios2linux.xswe";
      else
	return "ldscripts/nios2linux.xsw";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/nios2linux.xsce";
      else
	return "ldscripts/nios2linux.xsc";
    }
  else if (bfd_link_dll (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/nios2linux.xse";
      else
	return "ldscripts/nios2linux.xs";
    }
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/nios2linux.xwe";
      else
	return "ldscripts/nios2linux.xw";
    }
  else if (link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/nios2linux.xce";
      else
	return "ldscripts/nios2linux.xc";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/nios2linux.xe";
      else
	return "ldscripts/nios2linux.x";
    }
}
...
static bool
gldnios2linux_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
struct ld_emulation_xfer_struct ld_nios2linux_emulation =
{
  gldnios2linux_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldnios2linux_before_plugin_all_symbols_read,
  gldnios2linux_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldnios2linux_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  gldnios2linux_before_allocation,
  gldnios2linux_get_script,
  "nios2linux",
  "elf32-littlenios2",
  finish_default,
  nios2elf_create_output_section_statements,
  ldelf_open_dynamic_archive,
  ldelf_place_orphan,
  NULL,
  NULL,
  gldnios2linux_add_options,
  gldnios2linux_handle_option,
  NULL,
  NULL,
  ldelf_load_symbols,
  NULL,
