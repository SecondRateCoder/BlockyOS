...
#define bfd_arm_allocate_interworking_sections 	bfd_i386pe_posix_allocate_interworking_sections
#define bfd_arm_get_bfd_for_interworking 	bfd_i386pe_posix_get_bfd_for_interworking
#define bfd_arm_process_before_allocation 	bfd_i386pe_posix_process_before_allocation
#include "coff-arm.h"
#endif
...
#define MSIMAGEBASEOFF	2
  ...
  D(Subsystem,"__subsystem__", 7, false),
...
static bool
gldi386pe_posix_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
static void
gldi386pe_posix_before_plugin_all_symbols_read (void)
{
#ifdef DLL_SUPPORT
...
gldi386pe_posix_get_script (int *isfile)
{
  *isfile = 1;
  ...
  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/i386pe_posix.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/i386pe_posix.xr";
  else if (!config.text_read_only)
    return "ldscripts/i386pe_posix.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/i386pe_posix.xn";
  else if (link_info.pei386_auto_import == 1
	   && (MERGE_RDATA_V2 || link_info.pei386_runtime_pseudo_reloc != 2))
    return "ldscripts/i386pe_posix.xa";
  else
...
struct ld_emulation_xfer_struct ld_i386pe_posix_emulation =
{
  ...
  gldi386pe_posix_before_plugin_all_symbols_read,
