...
/* AIX emulation code for aix5ppc
...
static void gldaix5ppc_read_file (const char *, bool);
static void gldaix5ppc_free (void *);
static void gldaix5ppc_find_relocs (lang_statement_union_type *);
static void gldaix5ppc_find_exp_assignment (etree_type *);
...
static void
gldaix5ppc_before_parse (void)
{
  ldfile_set_output_arch ("powerpc", bfd_arch_powerpc);
...
static bool
gldaix5ppc_parse_args (int argc, char **argv)
{
  int indx;
...
static bool
gldaix5ppc_handle_option (int optc)
{
  bfd_signed_vma val;
  const char *end;
  ...
  switch (optc)
	...
	*t = 0;
    ...
    case OPTION_EXPORT:
      gldaix5ppc_read_file (optarg, false);
      break;
...
static bool
gldaix5ppc_unrecognized_file (lang_input_statement_type *entry)
{
  FILE *e;
  bool ret;
...
static void
gldaix5ppc_before_allocation (void)
{
  struct filelist *fl;
  ...
  for (fl = import_files; fl != NULL; fl = fl->next)
    gldaix5ppc_read_file (fl->name, true);
  ...
  lang_for_each_statement (gldaix5ppc_find_relocs);
...
static void
gldaix5ppc_after_allocation (void)
{
...
static char *
gldaix5ppc_choose_target (int argc, char **argv)
{
  int i, j, jmax;
...
static void
gldaix5ppc_read_file (const char *filename, bool import)
{
  struct obstack *o;
  FILE *f;
  int lineno;
  ...
  obstack_specify_allocation (o, 0, 0, xmalloc, gldaix5ppc_free);
...
static void
gldaix5ppc_find_relocs (lang_statement_union_type *s)
{
  ...
  if (s->header.type == lang_assignment_statement_enum)
    gldaix5ppc_find_exp_assignment (s->assignment_statement.exp);
}

/* Look through an expression for an assignment statement.  */

static void
gldaix5ppc_find_exp_assignment (etree_type *exp)
{
  struct bfd_link_hash_entry *h;
  ...
  switch (exp->type.node_class)
      ...
      if (strcmp (exp->assign.dst, ".") != 0)
	{
      ...
      gldaix5ppc_find_exp_assignment (exp->assign.src);
      break;

    case etree_binary:
      gldaix5ppc_find_exp_assignment (exp->binary.lhs);
      gldaix5ppc_find_exp_assignment (exp->binary.rhs);
      break;

    case etree_trinary:
      gldaix5ppc_find_exp_assignment (exp->trinary.cond);
      gldaix5ppc_find_exp_assignment (exp->trinary.lhs);
      gldaix5ppc_find_exp_assignment (exp->trinary.rhs);
      break;

    case etree_unary:
      gldaix5ppc_find_exp_assignment (exp->unary.child);
      break;
...
static char *
gldaix5ppc_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/aix5ppc.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/aix5ppc.xr";
  else if (!config.text_read_only)
    return "ldscripts/aix5ppc.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/aix5ppc.xn";
  else
    return "ldscripts/aix5ppc.x";
}

static void
gldaix5ppc_create_output_section_statements (void)
{
...
static void
gldaix5ppc_set_output_arch (void)
{
...
static bool
gldaix5ppc_open_dynamic_archive (const char *arch,
					   search_dirs_type *search,
...
static bool
gldaix5ppc_print_symbol (struct bfd_link_hash_entry *hash_entry,
				   void *ptr)
{
...
struct ld_emulation_xfer_struct ld_aix5ppc_emulation =
{
  ...
  after_parse_default,
  NULL,
  gldaix5ppc_after_open,
  after_check_relocs_default,
  before_place_orphans_default,
  gldaix5ppc_after_allocation,
  gldaix5ppc_set_output_arch,
  gldaix5ppc_choose_target,
  gldaix5ppc_before_allocation,
  gldaix5ppc_get_script,
  "aix5ppc",
  "aixcoff-rs6000",
  finish_default,
