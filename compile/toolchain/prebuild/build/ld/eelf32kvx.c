...
/* 32 bit ELF emulation code for elf32kvx
...
static void gldelf32kvx_before_parse (void);
static void gldelf32kvx_before_plugin_all_symbols_read
  (void);
static void gldelf32kvx_after_open (void);
static void gldelf32kvx_before_allocation (void);
static void gldelf32kvx_after_allocation (void);
...
static void
elf32_kvx_before_allocation (void)
{
...
/* Call-back for elf32_kvx_size_stubs.  */

/* Create a new stub section, and arrange for it to be linked
   immediately after INPUT_SECTION.  */

static asection *
elf32_kvx_add_stub_section (const char *stub_sec_name,
                                    asection *input_section)
{
  asection *stub_sec;
  flagword flags;
...
/* Another call-back for elf32_kvx_size_stubs.  */
...
build_section_lists (lang_statement_union_type *statement)
{
  if (statement->header.type == lang_input_section_enum)
	...
	elf32_kvx_next_input_section (& link_info, i);
    }
}

static void
gldelf32kvx_after_allocation (void)
{
  int ret;
  ...
  if (stub_file != NULL && !bfd_link_relocatable (&link_info))
    {
      ret = elf32_kvx_setup_section_lists (link_info.output_bfd,
						       &link_info);
      if (ret != 0)
	{
	  if (ret < 0)
	  ...
	  if (! elf32_kvx_size_stubs (link_info.output_bfd,
					  stub_file->the_bfd,
					  & link_info,
					  group_size,
					  & elf32_kvx_add_stub_section,
...
gldelf32kvx_finish (void)
{
  if (!bfd_link_relocatable (&link_info))
	...
	{
	  if (! elf32_kvx_build_stubs (& link_info))
...
kvx_elf_create_output_section_statements (void)
{
  ...
  if (!kvx_elf32_init_stub_bfd (&link_info, stub_file->the_bfd))
...
static void
gldelf32kvx_before_plugin_all_symbols_read (void)
{
...
static void
gldelf32kvx_before_allocation (void)
{
...
static char *
gldelf32kvx_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/elf32kvx.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/elf32kvx.xr";
  else if (!config.text_read_only)
    return "ldscripts/elf32kvx.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/elf32kvx.xn";
  else if (bfd_link_dll (&link_info) && link_info.combreloc
	   && link_info.relro && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/elf32kvx.xswe";
      else
	return "ldscripts/elf32kvx.xsw";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/elf32kvx.xsce";
      else
	return "ldscripts/elf32kvx.xsc";
    }
  else if (bfd_link_dll (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/elf32kvx.xse";
      else
	return "ldscripts/elf32kvx.xs";
    }
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/elf32kvx.xwe";
      else
	return "ldscripts/elf32kvx.xw";
    }
  else if (link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/elf32kvx.xce";
      else
	return "ldscripts/elf32kvx.xc";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/elf32kvx.xe";
      else
	return "ldscripts/elf32kvx.x";
    }
}
...
static bool
gldelf32kvx_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
struct ld_emulation_xfer_struct ld_elf32kvx_emulation =
{
  gldelf32kvx_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldelf32kvx_before_plugin_all_symbols_read,
  gldelf32kvx_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldelf32kvx_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  elf32_kvx_before_allocation,
  gldelf32kvx_get_script,
  "elf32kvx",
  "elf32-kvx",
  gldelf32kvx_finish,
