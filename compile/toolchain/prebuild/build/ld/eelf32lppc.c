...
/* 32 bit ELF emulation code for elf32lppc
...
static void gldelf32lppc_before_parse (void);
static void gldelf32lppc_before_plugin_all_symbols_read
  (void);
static void gldelf32lppc_after_open (void);
static void gldelf32lppc_before_allocation (void);
static void gldelf32lppc_after_allocation (void);
...
ppc_before_allocation (void)
{
  ...
  gldelf32lppc_before_allocation ();
  ...
  /* Turn on relaxation if executable sections have addresses that
  ...
  else if (!RELAXATION_DISABLED_BY_USER)
    {
	 ...
	 up gldelf32lppc_before_allocation sizing.  This
...
static void
gldelf32lppc_before_parse (void)
{
...
static void
gldelf32lppc_before_plugin_all_symbols_read (void)
{
...
static void
gldelf32lppc_before_allocation (void)
{
...
static void
gldelf32lppc_after_allocation (void)
{
...
static char *
gldelf32lppc_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/elf32lppc.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/elf32lppc.xr";
  else if (!config.text_read_only)
    return "ldscripts/elf32lppc.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/elf32lppc.xn";
	...
	return "ldscripts/elf32lppc.xdwe";
      else
	return "ldscripts/elf32lppc.xdw";
    }
  else if (bfd_link_pie (&link_info)
	   && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/elf32lppc.xdce";
      else
	return "ldscripts/elf32lppc.xdc";
    }
  else if (bfd_link_pie (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/elf32lppc.xde";
      else
	return "ldscripts/elf32lppc.xd";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc
	   && link_info.relro && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/elf32lppc.xswe";
      else
	return "ldscripts/elf32lppc.xsw";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/elf32lppc.xsce";
      else
	return "ldscripts/elf32lppc.xsc";
    }
  else if (bfd_link_dll (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/elf32lppc.xse";
      else
	return "ldscripts/elf32lppc.xs";
    }
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/elf32lppc.xwe";
      else
	return "ldscripts/elf32lppc.xw";
    }
  else if (link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/elf32lppc.xce";
      else
	return "ldscripts/elf32lppc.xc";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/elf32lppc.xe";
      else
	return "ldscripts/elf32lppc.x";
    }
}
...
static bool
gldelf32lppc_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
static void
gldelf32lppc_list_options (FILE * file)
{
...
struct ld_emulation_xfer_struct ld_elf32lppc_emulation =
{
  gldelf32lppc_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldelf32lppc_before_plugin_all_symbols_read,
  gldelf32lppc_after_open,
  ppc_after_check_relocs,
  ldelf_before_place_orphans,
  gldelf32lppc_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  ppc_before_allocation,
  gldelf32lppc_get_script,
  "elf32lppc",
  "elf32-powerpcle",
  ppc_finish,
