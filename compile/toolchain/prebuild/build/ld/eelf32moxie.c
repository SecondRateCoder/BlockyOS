...
/* 32 bit ELF emulation code for elf32moxie
...
static void gldelf32moxie_before_parse (void);
static void gldelf32moxie_before_plugin_all_symbols_read
  (void);
static void gldelf32moxie_after_open (void);
static void gldelf32moxie_before_allocation (void);
static void gldelf32moxie_after_allocation (void);


static void
gldelf32moxie_before_parse (void)
{
  ldfile_set_output_arch ("moxie", bfd_arch_moxie);
...
static void
gldelf32moxie_before_plugin_all_symbols_read (void)
{
...
static void
gldelf32moxie_before_allocation (void)
{
...
static void
gldelf32moxie_after_allocation (void)
{
...
static char *
gldelf32moxie_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/elf32moxie.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/elf32moxie.xr";
  else if (!config.text_read_only)
    return "ldscripts/elf32moxie.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/elf32moxie.xn";
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/elf32moxie.xwe";
      else
	return "ldscripts/elf32moxie.xw";
    }
  else if (link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/elf32moxie.xce";
      else
	return "ldscripts/elf32moxie.xc";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/elf32moxie.xe";
      else
	return "ldscripts/elf32moxie.x";
    }
}
...
static bool
gldelf32moxie_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
struct ld_emulation_xfer_struct ld_elf32moxie_emulation =
{
  gldelf32moxie_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldelf32moxie_before_plugin_all_symbols_read,
  gldelf32moxie_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldelf32moxie_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  gldelf32moxie_before_allocation,
  gldelf32moxie_get_script,
  "elf32moxie",
  "elf32-bigmoxie",
  finish_default,
  NULL,
