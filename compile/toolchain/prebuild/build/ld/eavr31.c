...
/* 32 bit ELF emulation code for avr31
...
static void gldavr31_before_parse (void);
static void gldavr31_before_plugin_all_symbols_read
  (void);
static void gldavr31_after_open (void);
static void gldavr31_before_allocation (void);
static void gldavr31_after_allocation (void);
...
static void
avr_elf_avr31_before_allocation (void)
{
  int ret;

  gldavr31_before_allocation ();
  ...
  if (strcmp ("avr31", "avr6") != 0
      && strcmp ("avr31", "avrxmega6") != 0
      && strcmp ("avr31", "avrxmega7") != 0)
    avr_no_stubs = true;
...
static void
gldavr31_before_parse (void)
{
  ldfile_set_output_arch ("avr:31", bfd_arch_avr);
...
static void
gldavr31_before_plugin_all_symbols_read (void)
{
...
static void
gldavr31_before_allocation (void)
{
...
static void
gldavr31_after_allocation (void)
{
...
static char *
gldavr31_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/avr31.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/avr31.xr";
  else if (!config.text_read_only)
    return "ldscripts/avr31.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/avr31.xn";
  else
    {
      if (link_info.separate_code)
	return "ldscripts/avr31.xe";
      else
	return "ldscripts/avr31.x";
    }
}
...
static bool
gldavr31_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
static void
gldavr31_list_options (FILE * file)
{
...
struct ld_emulation_xfer_struct ld_avr31_emulation =
{
  avr_elf_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldavr31_before_plugin_all_symbols_read,
  gldavr31_after_open,
  ...
  avr_elf_avr31_before_allocation,
  gldavr31_get_script,
  "avr31",
  "elf32-avr",
  avr_finish,
  ...
  NULL,
  gldavr31_add_options,
  gldavr31_handle_option,
  NULL,
  gldavr31_list_options,
  ldelf_load_symbols,
  NULL,
  NULL,
  NULL,
