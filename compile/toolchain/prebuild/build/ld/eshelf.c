...
/* 32 bit ELF emulation code for shelf
...
static void gldshelf_before_parse (void);
static void gldshelf_before_plugin_all_symbols_read
  (void);
static void gldshelf_after_open (void);
static void gldshelf_before_allocation (void);
static void gldshelf_after_allocation (void);


static void
gldshelf_before_parse (void)
{
...
static void
gldshelf_before_plugin_all_symbols_read (void)
{
...
static void
gldshelf_before_allocation (void)
{
...
static void
gldshelf_after_allocation (void)
{
...
static char *
gldshelf_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/shelf.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/shelf.xr";
  else if (!config.text_read_only)
    return "ldscripts/shelf.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/shelf.xn";
  else if (bfd_link_dll (&link_info) && link_info.combreloc
	   && link_info.relro && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/shelf.xswe";
      else
	return "ldscripts/shelf.xsw";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/shelf.xsce";
      else
	return "ldscripts/shelf.xsc";
    }
  else if (bfd_link_dll (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/shelf.xse";
      else
	return "ldscripts/shelf.xs";
    }
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/shelf.xwe";
      else
	return "ldscripts/shelf.xw";
    }
  else if (link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/shelf.xce";
      else
	return "ldscripts/shelf.xc";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/shelf.xe";
      else
	return "ldscripts/shelf.x";
    }
}
...
static bool
gldshelf_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
struct ld_emulation_xfer_struct ld_shelf_emulation =
{
  gldshelf_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldshelf_before_plugin_all_symbols_read,
  gldshelf_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldshelf_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  gldshelf_before_allocation,
  gldshelf_get_script,
  "shelf",
  "elf32-sh",
  finish_default,
