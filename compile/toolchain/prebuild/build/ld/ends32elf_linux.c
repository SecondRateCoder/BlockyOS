...
/* 32 bit ELF emulation code for nds32elf_linux
...
#define TARGET_IS_nds32elf_linux
...
static void gldnds32elf_linux_before_parse (void);
static void gldnds32elf_linux_before_plugin_all_symbols_read
  (void);
static void gldnds32elf_linux_after_open (void);
static void gldnds32elf_linux_before_allocation (void);
static void gldnds32elf_linux_after_allocation (void);
...
nds32_elf_after_allocation (void)
{
  ...
  gldnds32elf_linux_after_allocation ();
}


static void
gldnds32elf_linux_before_parse (void)
{
...
static void
gldnds32elf_linux_before_plugin_all_symbols_read (void)
{
...
static void
gldnds32elf_linux_after_open (void)
{
...
static void
gldnds32elf_linux_before_allocation (void)
{
...
static void
gldnds32elf_linux_after_allocation (void)
{
...
static char *
gldnds32elf_linux_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/nds32elf_linux.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/nds32elf_linux.xr";
  else if (!config.text_read_only)
    return "ldscripts/nds32elf_linux.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/nds32elf_linux.xn";
  else if (bfd_link_pie (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/nds32elf_linux.xde";
      else
	return "ldscripts/nds32elf_linux.xd";
    }
  else if (bfd_link_dll (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/nds32elf_linux.xse";
      else
	return "ldscripts/nds32elf_linux.xs";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/nds32elf_linux.xe";
      else
	return "ldscripts/nds32elf_linux.x";
    }
}
...
static bool
gldnds32elf_linux_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
static void
gldnds32elf_linux_list_options (FILE * file)
{
...
struct ld_emulation_xfer_struct ld_nds32elf_linux_emulation =
{
  gldnds32elf_linux_before_parse,
  syslib_default,
  hll_default,
  nds32_elf_after_parse,
  gldnds32elf_linux_before_plugin_all_symbols_read,
  ...
  gldnds32elf_linux_before_allocation,
  gldnds32elf_linux_get_script,
  "nds32elf_linux",
  "elf32-nds32le-linux",
  finish_default,
  ...
  NULL,
  gldnds32elf_linux_add_options,
  gldnds32elf_linux_handle_option,
  NULL,
  gldnds32elf_linux_list_options,
  ldelf_load_symbols,
  NULL,
  NULL,
  NULL,
