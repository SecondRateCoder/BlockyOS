...
static void gldppcmacos_read_file (const char *, bool);
static void gldppcmacos_free (void *);
static void gldppcmacos_find_relocs (lang_statement_union_type *);
static void gldppcmacos_find_exp_assignment (etree_type *);
...
static bool
gldppcmacos_parse_args (int argc, char **argv)
{
  int indx;
...
static bool
gldppcmacos_handle_option (int optc)
{
  bfd_signed_vma val;
  const char *end;
...
static bool
gldppcmacos_unrecognized_file (lang_input_statement_type *entry)
{
  FILE *e;
  bool ret;
...
static void
gldppcmacos_before_allocation (void)
{
  struct filelist *fl;
...
static void
gldppcmacos_after_allocation (void)
{
...
static char *
gldppcmacos_choose_target (int argc, char **argv)
{
  int i, j, jmax;
...
static void
gldppcmacos_read_file (const char *filename, bool import)
{
  struct obstack *o;
  FILE *f;
  int lineno;
...
static void
gldppcmacos_find_relocs (lang_statement_union_type *s)
{
...
static void
gldppcmacos_find_exp_assignment (etree_type *exp)
{
  struct bfd_link_hash_entry *h;
...
static char *
gldppcmacos_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/ppcmacos.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/ppcmacos.xr";
  else if (!config.text_read_only)
    return "ldscripts/ppcmacos.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/ppcmacos.xn";
  else
    return "ldscripts/ppcmacos.x";
}

static void
gldppcmacos_create_output_section_statements (void)
{
...
static void
gldppcmacos_set_output_arch (void)
{
...
static bool
gldppcmacos_open_dynamic_archive (const char *arch,
					   search_dirs_type *search,
...
static bool
gldppcmacos_print_symbol (struct bfd_link_hash_entry *hash_entry,
				   void *ptr)
{
...
struct ld_emulation_xfer_struct ld_ppcmacos_emulation =
{
  ...
  gldppcmacos_after_allocation,
  gldppcmacos_set_output_arch,
  gldppcmacos_choose_target,
