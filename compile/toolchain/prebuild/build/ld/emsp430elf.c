...
/* Emulate the original gld for the given msp430elf
...
static void
gldmsp430elf_before_allocation (void)
{
  if (bfd_link_relocatable (&link_info)
...
static void
gldmsp430elf_after_allocation (void)
{
  ldelf_map_segments (false);
}

static void
gldmsp430elf_before_parse (void)
{
#ifndef TARGET_			/* I.e., if not generic.  */
  ldfile_set_output_arch ("msp:14", bfd_arch_unknown);
#endif /* not TARGET_ */
...
static char *
gldmsp430elf_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/msp430elf.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/msp430elf.xr";
  else if (!config.text_read_only)
    return "ldscripts/msp430elf.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/msp430elf.xn";
  else
    return "ldscripts/msp430elf.x";
}
...
gldmsp430elf_place_orphan (asection * s,
				   const char * secname,
				   int constraint)
{
  char * lower_name;
...
static void
gldmsp430elf_list_options (FILE * file)
{
...
static bool
gldmsp430elf_handle_option (int optc)
{
  switch (optc)
    {
...
static void
gldmsp430elf_finish (void)
{
  finish_default ();
  check_array_section_alignment ();
}

struct ld_emulation_xfer_struct ld_msp430elf_emulation =
{
  gldmsp430elf_before_parse,
  syslib_default,
  hll_default,
  after_parse_default,
  NULL,
  ...
  gldmsp430elf_before_allocation,
  gldmsp430elf_get_script,
  "msp430elf",
  "elf32-msp430",
  gldmsp430elf_finish,
  NULL,
  NULL,
  gldmsp430elf_place_orphan,
  NULL,
  NULL,
  gldmsp430elf_add_options,
  gldmsp430elf_handle_option,
  NULL,
  gldmsp430elf_list_options,
  NULL,
  NULL,
  NULL,
  NULL,
