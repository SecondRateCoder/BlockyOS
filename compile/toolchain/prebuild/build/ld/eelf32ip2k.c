...
/* 32 bit ELF emulation code for elf32ip2k
...
static void gldelf32ip2k_before_parse (void);
static void gldelf32ip2k_before_plugin_all_symbols_read
  (void);
static void gldelf32ip2k_after_open (void);
static void gldelf32ip2k_before_allocation (void);
static void gldelf32ip2k_after_allocation (void);


static void
gldelf32ip2k_before_parse (void)
{
  ldfile_set_output_arch ("ip2k", bfd_arch_ip2k);
...
static void
gldelf32ip2k_before_plugin_all_symbols_read (void)
{
...
static void
gldelf32ip2k_before_allocation (void)
{
...
static void
gldelf32ip2k_after_allocation (void)
{
...
static char *
gldelf32ip2k_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/elf32ip2k.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/elf32ip2k.xr";
  else if (!config.text_read_only)
    return "ldscripts/elf32ip2k.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/elf32ip2k.xn";
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/elf32ip2k.xwe";
      else
	return "ldscripts/elf32ip2k.xw";
    }
  else if (link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/elf32ip2k.xce";
      else
	return "ldscripts/elf32ip2k.xc";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/elf32ip2k.xe";
      else
	return "ldscripts/elf32ip2k.x";
    }
}
...
static bool
gldelf32ip2k_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
struct ld_emulation_xfer_struct ld_elf32ip2k_emulation =
{
  gldelf32ip2k_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldelf32ip2k_before_plugin_all_symbols_read,
  gldelf32ip2k_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldelf32ip2k_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  gldelf32ip2k_before_allocation,
  gldelf32ip2k_get_script,
  "elf32ip2k",
  "elf32-ip2k",
  finish_default,
  NULL,
