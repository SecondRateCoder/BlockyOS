...
/* 32 bit ELF emulation code for criself
...
static void gldcriself_before_parse (void);
static void gldcriself_before_plugin_all_symbols_read
  (void);
static void gldcriself_after_open (void);
static void gldcriself_before_allocation (void);
static void gldcriself_after_allocation (void);


static void
gldcriself_before_parse (void)
{
...
static void
gldcriself_before_plugin_all_symbols_read (void)
{
...
static void
gldcriself_before_allocation (void)
{
...
static void
gldcriself_after_allocation (void)
{
...
static char *
gldcriself_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/criself.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/criself.xr";
  else if (!config.text_read_only)
    return "ldscripts/criself.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/criself.xn";
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/criself.xwe";
      else
	return "ldscripts/criself.xw";
    }
  else if (link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/criself.xce";
      else
	return "ldscripts/criself.xc";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/criself.xe";
      else
	return "ldscripts/criself.x";
    }
}
...
static bool
gldcriself_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
struct ld_emulation_xfer_struct ld_criself_emulation =
{
  gldcriself_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldcriself_before_plugin_all_symbols_read,
  gldcriself_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldcriself_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  gldcriself_before_allocation,
  gldcriself_get_script,
  "criself",
  "elf32-us-cris",
  finish_default,
  NULL,
