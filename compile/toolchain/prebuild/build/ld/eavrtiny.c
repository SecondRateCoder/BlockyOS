...
/* 32 bit ELF emulation code for avrtiny
...
static void gldavrtiny_before_parse (void);
static void gldavrtiny_before_plugin_all_symbols_read
  (void);
static void gldavrtiny_after_open (void);
static void gldavrtiny_before_allocation (void);
static void gldavrtiny_after_allocation (void);
...
static void
avr_elf_avrtiny_before_allocation (void)
{
  int ret;

  gldavrtiny_before_allocation ();
  ...
  if (strcmp ("avrtiny", "avr6") != 0
      && strcmp ("avrtiny", "avrxmega6") != 0
      && strcmp ("avrtiny", "avrxmega7") != 0)
    avr_no_stubs = true;
...
static void
gldavrtiny_before_parse (void)
{
  ldfile_set_output_arch ("avr:100", bfd_arch_avr);
...
static void
gldavrtiny_before_plugin_all_symbols_read (void)
{
...
static void
gldavrtiny_before_allocation (void)
{
...
static void
gldavrtiny_after_allocation (void)
{
...
static char *
gldavrtiny_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/avrtiny.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/avrtiny.xr";
  else if (!config.text_read_only)
    return "ldscripts/avrtiny.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/avrtiny.xn";
  else
    {
      if (link_info.separate_code)
	return "ldscripts/avrtiny.xe";
      else
	return "ldscripts/avrtiny.x";
    }
}
...
static bool
gldavrtiny_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
static void
gldavrtiny_list_options (FILE * file)
{
...
struct ld_emulation_xfer_struct ld_avrtiny_emulation =
{
  avr_elf_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldavrtiny_before_plugin_all_symbols_read,
  gldavrtiny_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  avr_elf_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  avr_elf_avrtiny_before_allocation,
  gldavrtiny_get_script,
  "avrtiny",
  "elf32-avr",
  avr_finish,
  ...
  NULL,
  gldavrtiny_add_options,
  gldavrtiny_handle_option,
  NULL,
  gldavrtiny_list_options,
  ldelf_load_symbols,
  NULL,
  NULL,
  NULL,
