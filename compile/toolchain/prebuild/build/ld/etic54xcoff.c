...
static bool
gldtic54xcoff_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
gldtic54xcoff_get_script (int *isfile)
{
  *isfile = 1;
  ...
  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/tic54xcoff.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/tic54xcoff.xr";
  else if (!config.text_read_only)
    return "ldscripts/tic54xcoff.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/tic54xcoff.xn";
  else
