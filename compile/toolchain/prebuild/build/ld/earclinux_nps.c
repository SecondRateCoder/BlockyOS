...
/* 32 bit ELF emulation code for arclinux_nps
...
static void gldarclinux_nps_before_parse (void);
static void gldarclinux_nps_before_plugin_all_symbols_read
  (void);
static void gldarclinux_nps_after_open (void);
static void gldarclinux_nps_before_allocation (void);
static void gldarclinux_nps_after_allocation (void);


static void
gldarclinux_nps_before_parse (void)
{
...
static void
gldarclinux_nps_before_plugin_all_symbols_read (void)
{
...
static void
gldarclinux_nps_after_open (void)
{
...
static void
gldarclinux_nps_before_allocation (void)
{
...
static void
gldarclinux_nps_after_allocation (void)
{
...
static char *
gldarclinux_nps_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/arclinux_nps.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/arclinux_nps.xr";
  else if (!config.text_read_only)
    return "ldscripts/arclinux_nps.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/arclinux_nps.xn";
	...
	return "ldscripts/arclinux_nps.xdwe";
      else
	return "ldscripts/arclinux_nps.xdw";
    }
  else if (bfd_link_pie (&link_info)
	   && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/arclinux_nps.xdce";
      else
	return "ldscripts/arclinux_nps.xdc";
    }
  else if (bfd_link_pie (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/arclinux_nps.xde";
      else
	return "ldscripts/arclinux_nps.xd";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc
	   && link_info.relro && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/arclinux_nps.xswe";
      else
	return "ldscripts/arclinux_nps.xsw";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/arclinux_nps.xsce";
      else
	return "ldscripts/arclinux_nps.xsc";
    }
  else if (bfd_link_dll (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/arclinux_nps.xse";
      else
	return "ldscripts/arclinux_nps.xs";
    }
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/arclinux_nps.xwe";
      else
	return "ldscripts/arclinux_nps.xw";
    }
  else if (link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/arclinux_nps.xce";
      else
	return "ldscripts/arclinux_nps.xc";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/arclinux_nps.xe";
      else
	return "ldscripts/arclinux_nps.x";
    }
}
...
static bool
gldarclinux_nps_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
struct ld_emulation_xfer_struct ld_arclinux_nps_emulation =
{
  gldarclinux_nps_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldarclinux_nps_before_plugin_all_symbols_read,
  gldarclinux_nps_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldarclinux_nps_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  gldarclinux_nps_before_allocation,
  gldarclinux_nps_get_script,
  "arclinux_nps",
  "elf32-littlearc",
  finish_default,
  NULL,
