...
/* 32 bit ELF emulation code for v850
...
static void gldv850_before_parse (void);
static void gldv850_before_plugin_all_symbols_read
  (void);
static void gldv850_after_open (void);
static void gldv850_before_allocation (void);
static void gldv850_after_allocation (void);
...
static void
gldv850_before_parse (void)
{
  ldfile_set_output_arch ("v850", bfd_arch_v850);
...
static void
gldv850_before_plugin_all_symbols_read (void)
{
...
static void
gldv850_before_allocation (void)
{
...
static char *
gldv850_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/v850.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/v850.xr";
  else if (!config.text_read_only)
    return "ldscripts/v850.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/v850.xn";
  else
    {
      if (link_info.separate_code)
	return "ldscripts/v850.xe";
      else
	return "ldscripts/v850.x";
    }
}
...
static bool
gldv850_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
struct ld_emulation_xfer_struct ld_v850_emulation =
{
  gldv850_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldv850_before_plugin_all_symbols_read,
  v850_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldv850_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  gldv850_before_allocation,
  gldv850_get_script,
  "v850",
  "elf32-v850",
  finish_default,
