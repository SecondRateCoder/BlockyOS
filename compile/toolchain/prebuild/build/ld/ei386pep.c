...
is_underscoring (void)
{
  ...
  if (!bfd_get_target_info ("pei-x86-64", NULL, NULL, &u, NULL))
    bfd_get_target_info ("pe-x86-64", NULL, NULL, &u, NULL);
...
static bool
gldi386pep_handle_option (int optc)
{
  is_underscoring ();
  switch (optc)
    {
...
static void
gldi386pep_before_plugin_all_symbols_read (void)
{
#ifdef DLL_SUPPORT
...
gldi386pep_get_script (int *isfile)
{
  *isfile = 1;
  ...
  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/i386pep.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/i386pep.xr";
  else if (!config.text_read_only)
    return "ldscripts/i386pep.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/i386pep.xn";
  else if (link_info.pei386_auto_import == 1
	   && link_info.pei386_runtime_pseudo_reloc != 2)
    return "ldscripts/i386pep.xa";
  else
...
struct ld_emulation_xfer_struct ld_i386pep_emulation =
{
  ...
  gldi386pep_before_plugin_all_symbols_read,
