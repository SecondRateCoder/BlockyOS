...
/* 32 bit ELF emulation code for pruelf
...
static void gldpruelf_before_parse (void);
static void gldpruelf_before_plugin_all_symbols_read
  (void);
static void gldpruelf_after_open (void);
static void gldpruelf_before_allocation (void);
static void gldpruelf_after_allocation (void);
...
static void
gldpruelf_before_parse (void)
{
  ldfile_set_output_arch ("pru", bfd_arch_pru);
...
static void
gldpruelf_before_plugin_all_symbols_read (void)
{
...
static void
gldpruelf_before_allocation (void)
{
...
static void
gldpruelf_after_allocation (void)
{
...
static char *
gldpruelf_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/pruelf.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/pruelf.xr";
  else if (!config.text_read_only)
    return "ldscripts/pruelf.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/pruelf.xn";
  else
    {
      if (link_info.separate_code)
	return "ldscripts/pruelf.xe";
      else
	return "ldscripts/pruelf.x";
    }
}
...
static bool
gldpruelf_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
struct ld_emulation_xfer_struct ld_pruelf_emulation =
{
  gldpruelf_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldpruelf_before_plugin_all_symbols_read,
  gldpruelf_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldpruelf_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  pruelf_before_allocation,
  gldpruelf_get_script,
  "pruelf",
  "elf32-pru",
  finish_default,
  NULL,
