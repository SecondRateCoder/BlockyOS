...
/* 32 bit ELF emulation code for avr5
...
static void gldavr5_before_parse (void);
static void gldavr5_before_plugin_all_symbols_read
  (void);
static void gldavr5_after_open (void);
static void gldavr5_before_allocation (void);
static void gldavr5_after_allocation (void);
...
static void
avr_elf_avr5_before_allocation (void)
{
  int ret;

  gldavr5_before_allocation ();
  ...
  if (strcmp ("avr5", "avr6") != 0
      && strcmp ("avr5", "avrxmega6") != 0
      && strcmp ("avr5", "avrxmega7") != 0)
    avr_no_stubs = true;
...
static void
gldavr5_before_parse (void)
{
  ldfile_set_output_arch ("avr:5", bfd_arch_avr);
...
static void
gldavr5_before_plugin_all_symbols_read (void)
{
...
static void
gldavr5_before_allocation (void)
{
...
static char *
gldavr5_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/avr5.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/avr5.xr";
  else if (!config.text_read_only)
    return "ldscripts/avr5.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/avr5.xn";
  else
    {
      if (link_info.separate_code)
	return "ldscripts/avr5.xe";
      else
	return "ldscripts/avr5.x";
    }
}
...
static bool
gldavr5_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
static void
gldavr5_list_options (FILE * file)
{
...
struct ld_emulation_xfer_struct ld_avr5_emulation =
{
  avr_elf_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldavr5_before_plugin_all_symbols_read,
  gldavr5_after_open,
  ...
  avr_elf_avr5_before_allocation,
  gldavr5_get_script,
  "avr5",
  "elf32-avr",
  avr_finish,
  ...
  NULL,
  gldavr5_add_options,
  gldavr5_handle_option,
  NULL,
  gldavr5_list_options,
  ldelf_load_symbols,
  NULL,
  NULL,
  NULL,
