...
/* 32 bit ELF emulation code for i386moss
...
static void gldi386moss_before_parse (void);
static void gldi386moss_before_plugin_all_symbols_read
  (void);
static void gldi386moss_after_open (void);
static void gldi386moss_before_allocation (void);
static void gldi386moss_after_allocation (void);
...
static void
gldi386moss_before_plugin_all_symbols_read (void)
{
...
static void
gldi386moss_before_allocation (void)
{
...
static void
gldi386moss_after_allocation (void)
{
...
static char *
gldi386moss_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/i386moss.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/i386moss.xr";
  else if (!config.text_read_only)
    return "ldscripts/i386moss.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/i386moss.xn";
  else if (bfd_link_dll (&link_info) && link_info.combreloc
	   && link_info.relro && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/i386moss.xswe";
      else
	return "ldscripts/i386moss.xsw";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/i386moss.xsce";
      else
	return "ldscripts/i386moss.xsc";
    }
  else if (bfd_link_dll (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/i386moss.xse";
      else
	return "ldscripts/i386moss.xs";
    }
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/i386moss.xwe";
      else
	return "ldscripts/i386moss.xw";
    }
  else if (link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/i386moss.xce";
      else
	return "ldscripts/i386moss.xc";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/i386moss.xe";
      else
	return "ldscripts/i386moss.x";
    }
}
...
static bool
gldi386moss_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
struct ld_emulation_xfer_struct ld_i386moss_emulation =
{
  gldi386moss_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldi386moss_before_plugin_all_symbols_read,
  gldi386moss_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldi386moss_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  gldi386moss_before_allocation,
  gldi386moss_get_script,
  "i386moss",
  "elf32-i386",
  finish_default,
