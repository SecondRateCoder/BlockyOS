...
/* 32 bit ELF emulation code for cskyelf
...
static void gldcskyelf_before_parse (void);
static void gldcskyelf_before_plugin_all_symbols_read
  (void);
static void gldcskyelf_after_open (void);
static void gldcskyelf_before_allocation (void);
static void gldcskyelf_after_allocation (void);
...
static void
gldcskyelf_after_allocation (void)
{
...
static void
gldcskyelf_before_plugin_all_symbols_read (void)
{
...
static void
gldcskyelf_before_allocation (void)
{
...
static char *
gldcskyelf_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/cskyelf.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/cskyelf.xr";
  else if (!config.text_read_only)
    return "ldscripts/cskyelf.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/cskyelf.xn";
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/cskyelf.xwe";
      else
	return "ldscripts/cskyelf.xw";
    }
  else if (link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/cskyelf.xce";
      else
	return "ldscripts/cskyelf.xc";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/cskyelf.xe";
      else
	return "ldscripts/cskyelf.x";
    }
}
...
static bool
gldcskyelf_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
static void
gldcskyelf_list_options (FILE * file)
{
...
struct ld_emulation_xfer_struct ld_cskyelf_emulation =
{
  gldcskyelf_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldcskyelf_before_plugin_all_symbols_read,
  gldcskyelf_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldcskyelf_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  gldcskyelf_before_allocation,
  gldcskyelf_get_script,
  "cskyelf",
  "elf32-csky-little",
  gldcskyelf_finish,
  ...
  NULL,
  gldcskyelf_add_options,
  gldcskyelf_handle_option,
  NULL,
  gldcskyelf_list_options,
  ldelf_load_symbols,
  NULL,
  NULL,
  NULL,
