...
/* AIX emulation code for aix5rs6
...
static void gldaix5rs6_read_file (const char *, bool);
static void gldaix5rs6_free (void *);
static void gldaix5rs6_find_relocs (lang_statement_union_type *);
static void gldaix5rs6_find_exp_assignment (etree_type *);
...
static void
gldaix5rs6_before_parse (void)
{
  ldfile_set_output_arch ("rs6000", bfd_arch_rs6000);
...
static bool
gldaix5rs6_parse_args (int argc, char **argv)
{
  int indx;
...
static bool
gldaix5rs6_handle_option (int optc)
{
  bfd_signed_vma val;
  const char *end;
  ...
  switch (optc)
	...
	*t = 0;
    ...
    case OPTION_EXPORT:
      gldaix5rs6_read_file (optarg, false);
      break;
...
static bool
gldaix5rs6_unrecognized_file (lang_input_statement_type *entry)
{
  FILE *e;
  bool ret;
...
static void
gldaix5rs6_before_allocation (void)
{
  struct filelist *fl;
  ...
  for (fl = import_files; fl != NULL; fl = fl->next)
    gldaix5rs6_read_file (fl->name, true);
  ...
  lang_for_each_statement (gldaix5rs6_find_relocs);
...
static void
gldaix5rs6_after_allocation (void)
{
...
static char *
gldaix5rs6_choose_target (int argc, char **argv)
{
  int i, j, jmax;
...
static void
gldaix5rs6_read_file (const char *filename, bool import)
{
  struct obstack *o;
  FILE *f;
  int lineno;
  ...
  obstack_specify_allocation (o, 0, 0, xmalloc, gldaix5rs6_free);
...
static void
gldaix5rs6_find_relocs (lang_statement_union_type *s)
{
  ...
  if (s->header.type == lang_assignment_statement_enum)
    gldaix5rs6_find_exp_assignment (s->assignment_statement.exp);
}

/* Look through an expression for an assignment statement.  */

static void
gldaix5rs6_find_exp_assignment (etree_type *exp)
{
  struct bfd_link_hash_entry *h;
  ...
  switch (exp->type.node_class)
      ...
      if (strcmp (exp->assign.dst, ".") != 0)
	{
      ...
      gldaix5rs6_find_exp_assignment (exp->assign.src);
      break;

    case etree_binary:
      gldaix5rs6_find_exp_assignment (exp->binary.lhs);
      gldaix5rs6_find_exp_assignment (exp->binary.rhs);
      break;

    case etree_trinary:
      gldaix5rs6_find_exp_assignment (exp->trinary.cond);
      gldaix5rs6_find_exp_assignment (exp->trinary.lhs);
      gldaix5rs6_find_exp_assignment (exp->trinary.rhs);
      break;

    case etree_unary:
      gldaix5rs6_find_exp_assignment (exp->unary.child);
      break;
...
static char *
gldaix5rs6_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/aix5rs6.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/aix5rs6.xr";
  else if (!config.text_read_only)
    return "ldscripts/aix5rs6.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/aix5rs6.xn";
  else
    return "ldscripts/aix5rs6.x";
}

static void
gldaix5rs6_create_output_section_statements (void)
{
...
static void
gldaix5rs6_set_output_arch (void)
{
...
static bool
gldaix5rs6_open_dynamic_archive (const char *arch,
					   search_dirs_type *search,
...
static bool
gldaix5rs6_print_symbol (struct bfd_link_hash_entry *hash_entry,
				   void *ptr)
{
...
struct ld_emulation_xfer_struct ld_aix5rs6_emulation =
{
  ...
  after_parse_default,
  NULL,
  gldaix5rs6_after_open,
  after_check_relocs_default,
  before_place_orphans_default,
  gldaix5rs6_after_allocation,
  gldaix5rs6_set_output_arch,
  gldaix5rs6_choose_target,
  gldaix5rs6_before_allocation,
  gldaix5rs6_get_script,
  "aix5rs6",
  "aixcoff-rs6000",
  finish_default,
