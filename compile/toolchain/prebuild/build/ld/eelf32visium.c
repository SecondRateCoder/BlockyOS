...
/* 32 bit ELF emulation code for elf32visium
...
static void gldelf32visium_before_parse (void);
static void gldelf32visium_before_plugin_all_symbols_read
  (void);
static void gldelf32visium_after_open (void);
static void gldelf32visium_before_allocation (void);
static void gldelf32visium_after_allocation (void);


static void
gldelf32visium_before_parse (void)
{
  ldfile_set_output_arch ("visium", bfd_arch_visium);
...
static void
gldelf32visium_before_plugin_all_symbols_read (void)
{
...
static void
gldelf32visium_after_open (void)
{
...
static void
gldelf32visium_before_allocation (void)
{
...
static void
gldelf32visium_after_allocation (void)
{
...
static char *
gldelf32visium_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/elf32visium.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/elf32visium.xr";
  else if (!config.text_read_only)
    return "ldscripts/elf32visium.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/elf32visium.xn";
  else
    {
      if (link_info.separate_code)
	return "ldscripts/elf32visium.xe";
      else
	return "ldscripts/elf32visium.x";
    }
}
...
static bool
gldelf32visium_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
struct ld_emulation_xfer_struct ld_elf32visium_emulation =
{
  gldelf32visium_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldelf32visium_before_plugin_all_symbols_read,
  gldelf32visium_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldelf32visium_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  gldelf32visium_before_allocation,
  gldelf32visium_get_script,
  "elf32visium",
  "elf32-visium",
  finish_default,
  NULL,
