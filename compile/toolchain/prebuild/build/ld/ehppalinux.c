...
/* 32 bit ELF emulation code for hppalinux
...
static void gldhppalinux_before_parse (void);
static void gldhppalinux_before_plugin_all_symbols_read
  (void);
static void gldhppalinux_after_open (void);
static void gldhppalinux_before_allocation (void);
static void gldhppalinux_after_allocation (void);
...
static void
gldhppalinux_after_allocation (void)
{
  int ret;
...
static void
gldhppalinux_before_parse (void)
{
  ldfile_set_output_arch ("hppa:hppa1.1", bfd_arch_hppa);
...
static void
gldhppalinux_before_plugin_all_symbols_read (void)
{
...
static void
gldhppalinux_before_allocation (void)
{
...
static char *
gldhppalinux_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/hppalinux.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/hppalinux.xr";
  else if (!config.text_read_only)
    return "ldscripts/hppalinux.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/hppalinux.xn";
	...
	return "ldscripts/hppalinux.xdwe";
      else
	return "ldscripts/hppalinux.xdw";
    }
  else if (bfd_link_pie (&link_info)
	   && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/hppalinux.xdce";
      else
	return "ldscripts/hppalinux.xdc";
    }
  else if (bfd_link_pie (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/hppalinux.xde";
      else
	return "ldscripts/hppalinux.xd";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc
	   && link_info.relro && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/hppalinux.xswe";
      else
	return "ldscripts/hppalinux.xsw";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/hppalinux.xsce";
      else
	return "ldscripts/hppalinux.xsc";
    }
  else if (bfd_link_dll (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/hppalinux.xse";
      else
	return "ldscripts/hppalinux.xs";
    }
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/hppalinux.xwe";
      else
	return "ldscripts/hppalinux.xw";
    }
  else if (link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/hppalinux.xce";
      else
	return "ldscripts/hppalinux.xc";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/hppalinux.xe";
      else
	return "ldscripts/hppalinux.x";
    }
}
...
static bool
gldhppalinux_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
static void
gldhppalinux_list_options (FILE * file)
{
...
struct ld_emulation_xfer_struct ld_hppalinux_emulation =
{
  gldhppalinux_before_parse,
  syslib_default,
  hll_default,
  hppaelf_after_parse,
  gldhppalinux_before_plugin_all_symbols_read,
  gldhppalinux_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldhppalinux_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  gldhppalinux_before_allocation,
  gldhppalinux_get_script,
  "hppalinux",
  "elf32-hppa-linux",
  finish_default,
  ...
  NULL,
  gldhppalinux_add_options,
  gldhppalinux_handle_option,
  NULL,
  gldhppalinux_list_options,
  ldelf_load_symbols,
  NULL,
  NULL,
  NULL,
