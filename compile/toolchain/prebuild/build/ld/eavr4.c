...
/* 32 bit ELF emulation code for avr4
...
static void gldavr4_before_parse (void);
static void gldavr4_before_plugin_all_symbols_read
  (void);
static void gldavr4_after_open (void);
static void gldavr4_before_allocation (void);
static void gldavr4_after_allocation (void);
...
static void
avr_elf_avr4_before_allocation (void)
{
  int ret;

  gldavr4_before_allocation ();
  ...
  if (strcmp ("avr4", "avr6") != 0
      && strcmp ("avr4", "avrxmega6") != 0
      && strcmp ("avr4", "avrxmega7") != 0)
    avr_no_stubs = true;
...
static void
gldavr4_before_parse (void)
{
  ldfile_set_output_arch ("avr:4", bfd_arch_avr);
...
static void
gldavr4_before_plugin_all_symbols_read (void)
{
...
static void
gldavr4_before_allocation (void)
{
...
static char *
gldavr4_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/avr4.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/avr4.xr";
  else if (!config.text_read_only)
    return "ldscripts/avr4.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/avr4.xn";
  else
    {
      if (link_info.separate_code)
	return "ldscripts/avr4.xe";
      else
	return "ldscripts/avr4.x";
    }
}
...
static bool
gldavr4_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
static void
gldavr4_list_options (FILE * file)
{
...
struct ld_emulation_xfer_struct ld_avr4_emulation =
{
  avr_elf_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldavr4_before_plugin_all_symbols_read,
  gldavr4_after_open,
  ...
  avr_elf_avr4_before_allocation,
  gldavr4_get_script,
  "avr4",
  "elf32-avr",
  avr_finish,
  ...
  NULL,
  gldavr4_add_options,
  gldavr4_handle_option,
  NULL,
  gldavr4_list_options,
  ldelf_load_symbols,
  NULL,
  NULL,
  NULL,
