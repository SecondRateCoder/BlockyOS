...
gldelf32rx_get_script (int *isfile)
{
  *isfile = 1;
  ...
  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/elf32rx.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/elf32rx.xr";
  else if (!config.text_read_only)
    return "ldscripts/elf32rx.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/elf32rx.xn";
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/elf32rx.xwe";
      else
	return "ldscripts/elf32rx.xw";
    }
  else if (link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/elf32rx.xce";
      else
	return "ldscripts/elf32rx.xc";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/elf32rx.xe";
      else
	return "ldscripts/elf32rx.x";
    }
}
