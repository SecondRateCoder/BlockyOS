...
/* 32 bit ELF emulation code for avr6
...
static void gldavr6_before_parse (void);
static void gldavr6_before_plugin_all_symbols_read
  (void);
static void gldavr6_after_open (void);
static void gldavr6_before_allocation (void);
static void gldavr6_after_allocation (void);
...
static void
avr_elf_avr6_before_allocation (void)
{
  int ret;

  gldavr6_before_allocation ();
  ...
  if (strcmp ("avr6", "avr6") != 0
      && strcmp ("avr6", "avrxmega6") != 0
      && strcmp ("avr6", "avrxmega7") != 0)
    avr_no_stubs = true;
...
static void
gldavr6_before_parse (void)
{
  ldfile_set_output_arch ("avr:6", bfd_arch_avr);
...
static void
gldavr6_before_plugin_all_symbols_read (void)
{
...
static void
gldavr6_before_allocation (void)
{
...
static char *
gldavr6_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/avr6.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/avr6.xr";
  else if (!config.text_read_only)
    return "ldscripts/avr6.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/avr6.xn";
  else
    {
      if (link_info.separate_code)
	return "ldscripts/avr6.xe";
      else
	return "ldscripts/avr6.x";
    }
}
...
static bool
gldavr6_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
static void
gldavr6_list_options (FILE * file)
{
...
struct ld_emulation_xfer_struct ld_avr6_emulation =
{
  avr_elf_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldavr6_before_plugin_all_symbols_read,
  gldavr6_after_open,
  ...
  avr_elf_avr6_before_allocation,
  gldavr6_get_script,
  "avr6",
  "elf32-avr",
  avr_finish,
  ...
  NULL,
  gldavr6_add_options,
  gldavr6_handle_option,
  NULL,
  gldavr6_list_options,
  ldelf_load_symbols,
  NULL,
  NULL,
  NULL,
