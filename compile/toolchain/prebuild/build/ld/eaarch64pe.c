...
#define DEFAULT_DLL_CHARACTERISTICS	(0 ? 0 : 					   IMAGE_DLL_CHARACTERISTICS_DYNAMIC_BASE 					 | IMAGE_DLL_CHARACTERISTICS_HIGH_ENTROPY_VA   					 | IMAGE_DLL_CHARACTERISTICS_NX_COMPAT)
...
#define NT_EXE_IMAGE_BASE   ((bfd_vma) (0 ? 0x100400000LL 					: 0x140000000LL))
#undef NT_DLL_IMAGE_BASE
#define NT_DLL_IMAGE_BASE   ((bfd_vma) (0 ? 0x400000000LL 					: 0x180000000LL))
#undef NT_DLL_AUTO_IMAGE_BASE
#define NT_DLL_AUTO_IMAGE_BASE   ((bfd_vma) (0 ? 0x400000000LL 					: 0x1C0000000LL))
#undef NT_DLL_AUTO_IMAGE_MASK
#define NT_DLL_AUTO_IMAGE_MASK   ((bfd_vma) (0 ? 0x1ffff0000LL 					: 0x1ffff0000LL))
#else
#undef  NT_EXE_IMAGE_BASE
#define NT_EXE_IMAGE_BASE   ((bfd_vma) (0 ? 0x100010000LL 					: 0x10000LL))
#undef NT_DLL_IMAGE_BASE
#define NT_DLL_IMAGE_BASE   ((bfd_vma) (0 ? 0x110000000LL 					: 0x10000000LL))
#undef NT_DLL_AUTO_IMAGE_BASE
#define NT_DLL_AUTO_IMAGE_BASE   ((bfd_vma) (0 ? 0x120000000LL 					: 0x61300000LL))
#undef NT_DLL_AUTO_IMAGE_MASK
#define NT_DLL_AUTO_IMAGE_MASK   ((bfd_vma) (0 ? 0x0ffff0000LL 					: 0x0ffc0000LL))
#undef  PE_DEF_SECTION_ALIGNMENT
...
static int dll;
static int pep_subsystem = PE_DEF_SUBSYSTEM;
...
is_underscoring (void)
{
  ...
  if (!bfd_get_target_info ("pei-aarch64-little", NULL, NULL, &u, NULL))
    bfd_get_target_info ("pe-aarch64-little", NULL, NULL, &u, NULL);
...
static void
gldaarch64pe_before_parse (void)
{
  is_underscoring ();
  ldfile_set_output_arch ("aarch64", bfd_arch_aarch64);
...
#define GET_INIT_SYMBOL_NAME(IDX)   (init[(IDX)].symbol    + ((!init[(IDX)].is_c_symbol || is_underscoring () == 1) ? 0 : 1))
...
static void
gldaarch64pe_list_options (FILE *file)
{
...
static bool
gldaarch64pe_handle_option (int optc)
{
  is_underscoring ();
  switch (optc)
    {
...
static void
gldaarch64pe_before_plugin_all_symbols_read (void)
{
#ifdef DLL_SUPPORT
...

static void
gldaarch64pe_before_allocation (void)
{
  is_underscoring ();
...
static bool
gldaarch64pe_unrecognized_file (lang_input_statement_type *entry ATTRIBUTE_UNUSED)
{
#ifdef DLL_SUPPORT
...
static bool
gldaarch64pe_recognized_file (lang_input_statement_type *entry ATTRIBUTE_UNUSED)
{
  is_underscoring ();
#ifdef DLL_SUPPORT
#ifdef TARGET_IS_i386pep
...
gldaarch64pe_place_orphan (asection *s,
				    const char *secname,
				    int constraint)
{
...
static bool
gldaarch64pe_open_dynamic_archive
...
static int
gldaarch64pe_find_potential_libraries
...

static char *
gldaarch64pe_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/aarch64pe.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/aarch64pe.xr";
  else if (!config.text_read_only)
    return "ldscripts/aarch64pe.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/aarch64pe.xn";
  else if (link_info.pei386_auto_import == 1
	   && link_info.pei386_runtime_pseudo_reloc != 2)
    return "ldscripts/aarch64pe.xa";
  else
    return "ldscripts/aarch64pe.x";
}

struct ld_emulation_xfer_struct ld_aarch64pe_emulation =
{
  gldaarch64pe_before_parse,
  syslib_default,
  hll_default,
  gldaarch64pe_after_parse,
  gldaarch64pe_before_plugin_all_symbols_read,
  gldaarch64pe_after_open,
  after_check_relocs_default,
  before_place_orphans_default,
  after_allocation_default,
  set_output_arch_default,
  ldemul_default_target,
  gldaarch64pe_before_allocation,
  gldaarch64pe_get_script,
  "aarch64pe",
  "pei-aarch64-little",
  gldaarch64pe_finish,
  NULL,
  gldaarch64pe_open_dynamic_archive,
  gldaarch64pe_place_orphan,
  gldaarch64pe_set_symbols,
  NULL,
  gldaarch64pe_add_options,
  gldaarch64pe_handle_option,
  gldaarch64pe_unrecognized_file,
  gldaarch64pe_list_options,
  gldaarch64pe_recognized_file,
  gldaarch64pe_find_potential_libraries,
  NULL,
  NULL,
  NULL,
