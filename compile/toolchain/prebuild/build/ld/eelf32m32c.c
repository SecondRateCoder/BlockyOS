...
/* 32 bit ELF emulation code for elf32m32c
...
static void gldelf32m32c_before_parse (void);
static void gldelf32m32c_before_plugin_all_symbols_read
  (void);
static void gldelf32m32c_after_open (void);
static void gldelf32m32c_before_allocation (void);
static void gldelf32m32c_after_allocation (void);


static void
need_relax_elf32m32c_before_allocation (void)
{
  /* Call main function; we're just extending it.  */
  gldelf32m32c_before_allocation ();
...
static void
gldelf32m32c_before_parse (void)
{
  ldfile_set_output_arch ("m32c", bfd_arch_m32c);
...
static void
gldelf32m32c_before_plugin_all_symbols_read (void)
{
...
static void
gldelf32m32c_before_allocation (void)
{
...
static void
gldelf32m32c_after_allocation (void)
{
...
static char *
gldelf32m32c_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/elf32m32c.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/elf32m32c.xr";
  else if (!config.text_read_only)
    return "ldscripts/elf32m32c.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/elf32m32c.xn";
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/elf32m32c.xwe";
      else
	return "ldscripts/elf32m32c.xw";
    }
  else if (link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/elf32m32c.xce";
      else
	return "ldscripts/elf32m32c.xc";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/elf32m32c.xe";
      else
	return "ldscripts/elf32m32c.x";
    }
}
...
static bool
gldelf32m32c_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
struct ld_emulation_xfer_struct ld_elf32m32c_emulation =
{
  gldelf32m32c_before_parse,
  syslib_default,
  hll_default,
  ldelf_after_parse,
  gldelf32m32c_before_plugin_all_symbols_read,
  gldelf32m32c_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldelf32m32c_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  need_relax_elf32m32c_before_allocation,
  gldelf32m32c_get_script,
  "elf32m32c",
  "elf32-m32c",
  finish_default,
  NULL,
