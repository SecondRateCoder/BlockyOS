...
/* 32 bit ELF emulation code for hppaelf
...
static void gldhppaelf_before_parse (void);
static void gldhppaelf_before_plugin_all_symbols_read
  (void);
static void gldhppaelf_after_open (void);
static void gldhppaelf_before_allocation (void);
static void gldhppaelf_after_allocation (void);
...
static void
gldhppaelf_after_allocation (void)
{
  int ret;
...
static void
gldhppaelf_before_parse (void)
{
  ldfile_set_output_arch ("hppa", bfd_arch_hppa);
...
static void
gldhppaelf_before_plugin_all_symbols_read (void)
{
...
static void
gldhppaelf_before_allocation (void)
{
...
static char *
gldhppaelf_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/hppaelf.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/hppaelf.xr";
  else if (!config.text_read_only)
    return "ldscripts/hppaelf.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/hppaelf.xn";
  else
    {
      if (link_info.separate_code)
	return "ldscripts/hppaelf.xe";
      else
	return "ldscripts/hppaelf.x";
    }
}
...
static bool
gldhppaelf_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
static void
gldhppaelf_list_options (FILE * file)
{
...
{
  gldhppaelf_before_parse,
  syslib_default,
  hll_default,
  hppaelf_after_parse,
  gldhppaelf_before_plugin_all_symbols_read,
  gldhppaelf_after_open,
  after_check_relocs_default,
  ldelf_before_place_orphans,
  gldhppaelf_after_allocation,
  ldelf_set_output_arch,
  ldemul_default_target,
  gldhppaelf_before_allocation,
  gldhppaelf_get_script,
  "hppaelf",
  "elf32-hppa",
  finish_default,
  ...
  NULL,
  gldhppaelf_add_options,
  gldhppaelf_handle_option,
  NULL,
  gldhppaelf_list_options,
  ldelf_load_symbols,
  NULL,
  NULL,
  NULL,
