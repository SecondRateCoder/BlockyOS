...
SECTIONS
{
  ...
  PROVIDE (__executable_start = SEGMENT_START("text-segment", 0x100000)); . = SEGMENT_START("text-segment", 0x100000);
  .interp         : { *(.interp) }
  ...
  .init           :
  {
    *(.init$00); *(.init$0[1-9]); *(.init$[1-8][0-9]); *(.init$9[0-8])
    KEEP (*(SORT_NONE(.init)))
    *(.init$99)
  }
  .plt            : { *(.plt) }
