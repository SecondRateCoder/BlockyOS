...
static int blake2bp_init_leaf( blake2b_state *S, UINT64 outlen, UINT64 keylen, UINT64 offset )
{
  blake2b_param P[1];
...
static int blake2bp_init_root( blake2b_state *S, UINT64 outlen, UINT64 keylen )
{
  blake2b_param P[1];
  P->digest_length = (UINT8)outlen;
  P->key_length = (UINT8)keylen;
...
int blake2bp_update( blake2bp_state *S, const void *pin, UINT64 inlen )
{
  const unsigned char * in = (const unsigned char *)pin;
  UINT64 left = S->buflen;
  UINT64 fill = sizeof( S->buf ) - left;
  UINT64 i;

  if( left && inlen >= fill )
  {
    __memcpy( S->buf + left, in, fill );
  ...
  {
#if defined(_OPENMP)
    UINT64      i = omp_get_thread_num();
#endif
    UINT64 inlen__ = inlen;
...
{
  UINT8 hash[PARALLELISM_DEGREE][BLAKE2B_OUTBYTES];
  UINT64 i;
  ...
  for( i = 0; i < PARALLELISM_DEGREE; ++i )
  {
    ...
    {
      UINT64 left = S->buflen - i * BLAKE2B_BLOCKBYTES;
...
int blake2bp( void *out, UINT64 outlen, const void *in, UINT64 inlen, const void *key, UINT64 keylen )
{
...
#endif
  {
    ...
    {
      const UINT64 left = inlen__ - i * BLAKE2B_BLOCKBYTES;
      const UINT64 len = left <= BLAKE2B_BLOCKBYTES ? left : BLAKE2B_BLOCKBYTES;
...
int main( void )
{
  ...
  for( i = 0; i < BLAKE2_KAT_LENGTH; ++i )
  {
    ...
    if( 0 != __memcmp( hash, blake2bp_keyed_kat[i], BLAKE2B_OUTBYTES ) )
    {
      goto fail;
    }
  }
