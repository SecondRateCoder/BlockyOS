...
{
	UINT64 i;
	__memset( S, 0, sizeof( blake2s_state ) );
...
int blake2s_update( blake2s_state *S, const void *pin, UINT64 inlen )
{
	const unsigned char * in = (const unsigned char *)pin;
	if( inlen > 0 )
	{
		UINT64 left = S->buflen;
		UINT64 fill = BLAKE2S_BLOCKBYTES - left;
		if( inlen > fill )
		{
			S->buflen = 0;
...
{
	UINT8 buffer[BLAKE2S_OUTBYTES] = {0};
	UINT64 i;
	...
	__memset( S->buf + S->buflen, 0, BLAKE2S_BLOCKBYTES - S->buflen ); /* Padding */
	blake2s_compress( S, S->buf );

	for( i = 0; i < 8; ++i ) /* Output full hash to temp buffer */
		store32( buffer + sizeof( S->h[i] ) * i, S->h[i] );

	__memcpy( out, buffer, outlen );
...
int main( void )
{
	...
	for( i = 0; i < BLAKE2_KAT_LENGTH; ++i )
	{
		...
		if( 0 != __memcmp( hash, blake2s_keyed_kat[i], BLAKE2S_OUTBYTES ) )
		{
			goto fail;
		}
	}
