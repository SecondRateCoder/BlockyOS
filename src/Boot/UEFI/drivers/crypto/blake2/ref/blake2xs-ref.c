...
int blake2xs_final(blake2xs_state *S, void *out, UINT64 outlen) {
  ...
  blake2s_state C[1];
  blake2s_param P[1];
  UINT16 xof_length = load16(&S->P->xof_length);
  UINT8 root[BLAKE2S_BLOCKBYTES];
  UINT64 i;
  ...
  __memcpy(P, S->P, sizeof(blake2s_param));
  P->key_length = 0;
  P->fanout = 0;
  P->depth = 0;
  ...
  for (i = 0; outlen > 0; ++i) {
    const UINT64 block_size = (outlen < BLAKE2S_OUTBYTES) ? outlen : BLAKE2S_OUTBYTES;
    /* Initialize state */
    ...
    if (blake2s_final(C, (UINT8 *)out + i * BLAKE2S_OUTBYTES, block_size) < 0) {
        return -1;
    }
    outlen -= block_size;
  }
...
int main( void )
{
  ...
  for( outlen = 1; outlen <= BLAKE2_KAT_LENGTH; ++outlen )
  {
      ...
      if( 0 != __memcmp( hash, blake2xs_keyed_kat[outlen-1], outlen ) )
      {
        goto fail;
      }
  }
