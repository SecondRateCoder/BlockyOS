...
int blake2xb_init_key( blake2xb_state *S, const UINT64 outlen, const void *key, UINT64 keylen)
{
  ...
  S->P->node_depth    = 0;
  S->P->inner_length  = 0;
  __memset( S->P->reserved, 0, sizeof( S->P->reserved ) );
  __memset( S->P->salt,     0, sizeof( S->P->salt ) );
  __memset( S->P->personal, 0, sizeof( S->P->personal ) );
...
int blake2xb_final( blake2xb_state *S, void *out, UINT64 outlen) {
  ...
  blake2b_state C[1];
  blake2b_param P[1];
  UINT32 xof_length = load32(&S->P->xof_length);
  UINT8 root[BLAKE2B_BLOCKBYTES];
  UINT64 i;
  ...
  __memcpy(P, S->P, sizeof(blake2b_param));
  P->key_length = 0;
  P->fanout = 0;
  P->depth = 0;
  ...
  for (i = 0; outlen > 0; ++i) {
    const UINT64 block_size = (outlen < BLAKE2B_OUTBYTES) ? outlen : BLAKE2B_OUTBYTES;
    /* Initialize state */
    ...
    if (blake2b_final(C, (UINT8 *)out + i * BLAKE2B_OUTBYTES, block_size) < 0 ) {
        return -1;
    }
    outlen -= block_size;
  }
...
int main( void )
{
  ...
  {
      UINT8 hash[BLAKE2_KAT_LENGTH] = {0};
      if( blake2xb( hash, outlen, buf, BLAKE2_KAT_LENGTH, key, BLAKE2B_KEYBYTES ) < 0 ) {
        goto fail;
      }

      if( 0 != __memcmp( hash, blake2xb_keyed_kat[outlen-1], outlen ) )
      {
        goto fail;
      }
  }
  ...
  for(step = 1; step < BLAKE2B_BLOCKBYTES; ++step) {
    for (outlen = 1; outlen <= BLAKE2_KAT_LENGTH; ++outlen) {
      UINT8 hash[BLAKE2_KAT_LENGTH];
      blake2xb_state S;
      UINT8 * p = buf;
      UINT64 mlen = BLAKE2_KAT_LENGTH;
      int err = 0;
